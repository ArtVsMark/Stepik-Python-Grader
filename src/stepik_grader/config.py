...
#: Режимы сравнения вывода (issue #1111). ``stepik`` — как чекер платформы:
#: хвостовые пробелы строки и хвостовые пустые строки незначимы; ``strict`` —
#: побайтовая построчная сверка. Внутренние различия (порядок, регистр,
#: значащие пробелы, число содержательных строк) значимы в обоих режимах.
COMPARE_MODES = ("stepik", "strict")
...
class GraderConfig:
    ...
    # issue #920 (MET-1-06): класть ли исходник решения в промпт подсказки.
    # По умолчанию True — так канал работал всегда, и ровно это называют диалог
    # согласия и SECURITY.md; обещания продукта настройка не меняет. False
    # оставляет вердикт, diff, трейсбек и карточки: подсказка беднее, но есть,
    # а код не покидает машину. Прежде выбора не было вовсе — при том что
    # докстринг модуля утверждал обратное.
    ai_send_code: bool = True
    ...
    history_db_path: str = ""
    # issue #1111: как сравнивать вывод решения с ожиданием.
    # ``stepik`` (по умолчанию) — как чекер платформы: хвостовые пробелы в
    # конце строки и хвостовые пустые строки незначимы. ``strict`` — побайтово
    # построчно, прежнее поведение.
    # Дефолт выбран по внешнему эталону: на реальной базе курса решения,
    # ПРИНЯТЫЕ Stepik, получали у нас WA — расхождение только в хвостовом
    # пробеле и в «пустой вывод против пустой строки». Продукт грейдит задачи
    # Stepik, поэтому его эталон совпадения — платформенный; ``strict``
    # остаётся для тех, кому нужна побайтовая сверка (авторы задач, корпус).
    compare_mode: str = "stepik"
