...
#:
#: Четыре карточки (`permissionerror`, `open`, `pathlib.path`, `.iterdir`)
#: сюда вернулись после прогона на матрице: их примеры жёстко завязаны на
#: POSIX-пути (`/etc/shadow`, `/etc/hostname`, `/tmp`), и починка отступов лишь
#: делала их исполнимыми — на Windows такие пути не существуют, и пример падал
#: там, где раньше молча пропускался. Такой карточке нужен либо тег
#: `platform:posix`, либо кроссплатформенный пример (`tempfile` вместо `/tmp`):
#: это предметное решение, а не отступы. Пятой была `os` — она починена
#: (`tempfile.gettempdir()`, `os.path.*`), и остаток теперь без неё.
#:
#: 90 → 67 → 49 → 36 → 30 (issue #1454): партиями, руками. Первая — двадцать три карточки
#: с ОДНИМ открывателем блока, вторая — восемнадцать коротких с несколькими,
#: третья — тринадцать средних, четвёртая — шесть длинных из `op.json`.
#:
#: Автоматом это не делается, и попытка измерена: эвристика «после двоеточия
#: уровень +1» из блока никогда не выходит, поэтому она сворачивает весь пример
#: в лестницу вложенности — а `ast.parse` такой результат ПРИНИМАЕТ, и прогон
#: тоже (вложенные определения не исполняются): 80 карточек из 90 прошли
#: приёмку, верной не была ни одна. Приёмка вышла слабее чинимого дефекта, а
#: цена ошибки несимметрична — сломанный пример виден, «починенный» нет.
BUDGET = 30
