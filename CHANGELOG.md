...
### Added
- `--ai-hints` — opt-in AI explanation of failing (WA/RE) cases in CLI modes 1/2 via a BYOK OpenAI-compatible endpoint on bare `requests` (no new deps; ADR-0003). Off by default; unconfigured/network/timeout/invalid-key → silent skip with a hint, grading never fails. Config `[tool.stepik-grader]` `ai_base_url`/`ai_model`/`ai_api_key_env` (key read from env, never written/logged — redacted via `diag_log`); prompt grounded on `failure_kind`+diff+error card, answer marked AI-generated. Works with local (ollama) and cloud providers via one code path (#435).
