...
Раннеры GitHub Actions **не дают** `bwrap` поднять loopback в свежем network
namespace: `--unshare-net` завершается `RTM_NEWADDR: Operation not permitted`,
поэтому полноценный (с сетевой изоляцией) sandbox-прогон на обычном GHA-раннере
не стартует вовсе. Чтобы Linux-бэкенд всё же проверялся автоматически (а не
только локально), CI гоняет его в двух режимах:

- **`sandbox-linux`** (обязательный job) — исполняет реальный `bwrap` **без**
  `--unshare-net` через тест-only seam `LinuxSandboxRunner(..., unshare_net=False)`.
  Покрывает всё, что не зависит от сетевого namespace: запуск интерпретатора в
  песочнице (ловит регрессии usrmerge-обвязки), ФС-изоляцию, `RLIMIT_AS`
  (память), `output_size`, timeout. Покрытие `_linux.py` вливается в cross-OS
  combine. **`unshare_net=False` — исключительно тестовый seam**: прод-путь
  (`create_backend()`) всегда конструирует бэкенд с полной изоляцией, сетевой
  namespace в боевом коде не отключается.
- **`sandbox-linux-privileged`** (эксперимент, `continue-on-error`) — проверяет
  гипотезу, что privileged-контейнер снимает запрет на netns, и тогда гоняет
  полный набор **с** сетевой изоляцией.

**Сетевая изоляция** (`--unshare-net`) и **anti-fork-bomb** (`RLIMIT_NPROC`,
зависит от сброса kernel-ucounts) проверяются полным netns-классом на
локальной машине / self-hosted-раннере с рабочим netns, а также кросс-OS
эквивалентами в Windows/macOS-наборах — на бесплатных GHA-раннерах эти две
гарантии автоматически не тестируются (ограничение платформы, не пробел
реализации).
