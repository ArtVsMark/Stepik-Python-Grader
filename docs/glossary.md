...
В пакете лежит готовая база — `src/stepik_grader/glossary/data/*.json` (≈1388
карточек на момент правки: 601 `ready` видны по умолчанию + 787
черновиков-автодрафтов, скрытых из дефолтной выдачи ученика с issue #436;
пополняется волнами наполнения глоссария, эпик #363; по файлу на цветовую группу
`cg`), частично импортированная из внешнего
...
пополнения по-прежнему работают по ПОЛНОЙ базе. Итог: `data/drafts.json`
(изначально 832 автодрафта, доводятся до `ready` волнами эпика #363 — секция
«Строки (str)» уже наполнена) доводит покрытие офлайн-инвентаря до ~100%.
