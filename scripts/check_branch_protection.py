...
#: Обязательные проверки, дословно как в ruleset. Теперь их **одна** — переход
#: по issue #1420 завершён.
#:
#: ПОЧЕМУ ОДНА. Имя ячейки матрицы несёт версию Python и флаг эксперимента, то
#: есть меняется от чужого календаря: выход 3.14 из предрелиза добавил сюда три
#: строки (#1529), выход 3.15 добавит ещё три и уберёт другие. Каждый такой
#: сдвиг правится В ДВУХ местах сразу — здесь и во внешней настройке
#: репозитория, — а расходятся они молча: PR начинает ждать проверку, которой
#: больше нет, и ни один прогон об этом не говорит. Это и есть то, чего правило
#: 168 не велит держать в обязательных.
#:
#: :data:`AGGREGATE_CHECK` от календаря не зависит: имя постоянное, а состав он
#: выводит из ``ci.yml`` сам (``ci_aggregate.expected_checks``). Переименование
#: ячейки после этого перехода по-прежнему ловится — но уже прогоном, а не
#: настройкой, которую надо помнить.
#:
#: ПОРЯДОК ПЕРЕХОДА БЫЛ ТАКОЙ, и менять его было нельзя: агрегатор появился
#: (#1456) → владелец добавил его в обязательные, ничего не убирая → PR
#: мержились при нём, и он доказал, что зеленеет при красных предрелизных
#: ячейках и краснеет при красной обязательной → и только теперь четырнадцать
#: имён уходят отсюда и из ruleset. Обратный порядок оставил бы защиту без
#: проверок либо отправил PR в вечное ожидание, которое чинится снятием защиты.
EXPECTED_CHECKS: tuple[str, ...] = ("ci-complete",)
...
def check_matrix_names(text: str, expected: tuple[str, ...] = ()) -> list[str]:
    ...
    ПРЕДМЕТ ЭТОЙ ПРОВЕРКИ СМЕНИЛСЯ ВМЕСТЕ С ПЕРЕХОДОМ #1420. Раньше она держала
    список имён в соответствии с матрицей: пока ячейки перечислялись в
    обязательных, разъехаться они могли молча. Теперь имя обязательной проверки
    одно и постоянное, а состав выводит агрегатор из того же ``ci.yml``
    (``ci_aggregate.expected_checks``) — значит сверять список с матрицей больше
    незачем, а вот **возвращение матричного имени в список** стало находкой:
    оно тянет назад к состоянию, где каждый выпуск CPython правится в двух
    местах сразу и расходится молча.

    Матрица при этом обязана разбираться: агрегатор берёт состав оттуда, и
    неразобранная матрица означала бы пустой состав — вердикт, который ничего
    не ждёт.
    ...
    declared_matrix = sorted(name for name in declared if name.startswith("test ("))
    if declared_matrix:
        return [
            f"матричные имена вернулись в обязательные ({len(declared_matrix)}): "
            f"{', '.join(declared_matrix)}. Состав выводит {AGGREGATE_CHECK!r} из ci.yml; "
            "перечисленное имя надо править ещё и во внешней настройке, а расходятся "
            "они молча — PR ждёт проверку, которой больше нет"
        ]
    return []
