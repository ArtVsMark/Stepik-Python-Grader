...
issue #996 (``ARCH-3-01``): модуль живёт **вне** ``core/`` — как ``atomic_io``
(#551), ``db`` (#552) и ``stdio_encoding`` (#1108), и по той же причине.
Подпакеты ``glossary/`` и ``rules/`` по инварианту ``core/`` не импортируют
(CLAUDE.md § Архитектурные инварианты, ADR-0011), а ``rules/json_provider``
кеш нужен — пока модуль лежал в ``core/``, единственным способом им
воспользоваться было ребро ``rules → core``, ровно то, которое инвариант
запрещает. Общий инфра-хелпер на верхнем уровне доступен и подпакетам, и
