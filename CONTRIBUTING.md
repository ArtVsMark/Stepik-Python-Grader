...
> Динамическая версия (issue #162). Сборка (`pyproject.toml`) больше НЕ
> объявляет `version` статически — она вычисляется из git-тегов через
> `setuptools-scm` (`dynamic = ["version"]`), git — единственный источник
> истины. `stepik_grader.__version__` по-прежнему читается через
> `importlib.metadata`, то есть отражает версию, вычисленную при `pip install`/
> сборке. Формат метаданных пакета — PEP 440 (`setuptools-scm`,
> `version_scheme = "post-release"`):
>
> - на теге `vX.Y.0` → `X.Y.0` (совпадает с релизом);
> - вне тега (N коммитов после тега) → `X.Y.0.postN+g<hash>` — технически
>   отличимая dev-сборка.
>
> `scripts/version.py` остаётся справочным инструментом: печатает
> «логический» `X.Y.N` по схеме проекта (тот же git-источник, другой формат) —
> удобно для тегирования и человекочитаемого счётчика коммитов.
> `scripts/check_version_consistency.py` (CI, issue #165) следит, чтобы
> статический source-of-truth не вернулся в `pyproject.toml`, а «текущая
> версия» в `CHECKPOINT.md`/`CHANGELOG.md` не расходилась с последним тегом.
