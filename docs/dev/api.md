...
> issue #140/#156/#157) — в [server-mode.md](design/server-mode.md).
>
...
`median_s`/`max_s` — числами в секундах (issue #731: диаграмме сравнения нужны
...
Issue #686 расширил охват до «любого совпадения с глоссарием, кроме имён
переменных»:
...
(`renderCommandButtons` в `static/grade.js`) — единственную оставшуюся
поверхность: command palette снята в #658, нижние сценарные кнопки убраны из
приложения целиком. Сам эндпоинт и реестр `COMMANDS` живы.
...
эпик #751, issue #754. Возвращает **ссылку на заполненную форму issue** на
GitHub (`.github/ISSUE_TEMPLATE/*.yml`, prefilled query-параметры по `id`
полей) и предпросмотр того, что в неё уйдёт.
