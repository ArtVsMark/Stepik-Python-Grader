...
**Запуск без командной строки (лаунчер, issue #661).** Если даже `--serve` —
барьер, есть окно-лаунчер: `stepik-grader-gui` (после установки; на Windows это
ярлык без консольного окна) или `python -m stepik_grader.launcher`. В окне —
явный выбор варианта запуска («Простой сервер» / «Сервер с изоляцией `--sandbox`»),
выбор порта (с проверкой «занят»), рабочая папка, кнопка «Запустить» (поднимает сервер отдельным
