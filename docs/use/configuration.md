...
> **Формат 3: края блока.** Пустая строка в начале или в конце блока
> `# TEST_N:` теряется всегда — граница блока задаётся маркером, и отбивку
> перед следующим маркером от данных не отличить. Задачу, где пустая строка по
> краям ожидаемого вывода значима, задавайте форматом 1 или 2. Пробелы по краям
> блока сейчас теряются тоже, поэтому решение, печатающее фигуру с отступами
> (ёлочка, таблица), получает `WA` при верном выводе — в форматах 1 и 2 такие
> пробелы сохраняются, и задачи с выравниванием надёжнее задавать ими.
