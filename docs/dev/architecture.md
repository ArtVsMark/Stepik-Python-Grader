...
| `__main__.py` (top-level) | Application | Точка входа `python -m stepik_grader` — делегирует в ту же `cli.main()`, что и консольная команда |
...
| `cli/exit_codes.py` | Interface / CLI (leaf) | Коды возврата CLI как машинный контракт: без них `--output json` для CI отвечал бы нулём и на провале |
| `cli/prompts.py` | Application / CLI (leaf) | Два набора утвердительных ответов, и разница смысловая: согласие с предложенным действием против явного подтверждения необратимого |
...
| `web/settings_adapter.py` | Application / Web | `read_settings`/`set_flag` — тонкий адаптер над `core/user_settings`: слой маршрутов переключает `onboarding_seen`/`ai_hint_consent` через него, а не читает файл настроек сам. Он же знает **получателя** согласия на AI-подсказку (`ai_consent_recipient`): адрес провайдера резолвится в адаптере, потому что роутеру ходить в `core`/`config` нельзя (ADR-0010) |
...
| `web/navigation_adapter.py` | Application / Web | Инвентарь скачанных задач со статусами: дерево на диске плюс агрегат прогресса из истории |
| `web/statement_adapter.py` | Application / Web | Условие скачанной задачи: заголовок из `meta.json`, тело после очистки, картинки с диска — сам ничего не решает и в сеть не ходит |
| `web/usage_adapter.py` | Application / Web | Журнал прогонов для веб-слоя: маршрут не знает формы `ExportResult`, потому что ходит в ядро только через адаптер (ARCH-07) |
| `ide.py` | Application / IDE | IDE-интеграция `--init-vscode`: генерация конфигов VS Code (tasks/launch) |
| `launcher.py` | Application / GUI | GUI-лаунчер веб-интерфейса без командной строки: tkinter-окно (выбор запуска простой/с изоляцией `--sandbox`, порт, папка, Запустить/Остановить, статус) поднимает `--serve` **отдельным процессом**; gui-script `stepik-grader-gui`. Из проекта тянет только `stdio_encoding`, `build_info` (строка версии), `config.workspace_root` (общий корень настроек — окно стартует там же, где его видят CLI и веб), `core/user_settings` (память выбора между запусками) и `core/settings_resolver` (вкладка «Дополнительно») — ядро прогона в процесс окна не импортируется |
...
| `core/shortcut.py` | Infrastructure / Utilities (leaf) | Ярлык запуска веб-лаунчера в системе — чтобы поднять интерфейс не требовалось помнить каталог и команду |
...
| `core/spawn.py` | Infrastructure / Utilities (leaf) | `guarded_popen`/`guarded_run` — порождение процесса с дедлайном на САМ запуск. `timeout=` у `subprocess` покрывает уже стартовавший процесс, а подвиснуть можно раньше — в `Popen.__init__`, на чтении errpipe после fork/exec; тогда вызывающий поток блокируется навсегда (под `--serve` — занятый воркер, которого никто не отменит). Запуск уходит в daemon-поток с дедлайном; процесс, родившийся после сдачи, убивается, чтобы не остался без надзора. Stdlib-only, project-импортов нет |
| `core/run_dir.py` | Infrastructure / Utilities | Временный каталог на один прогон с устойчивой уборкой: удаление не падает, если файл ещё держит не завершившийся процесс; осиротевшие каталоги подметаются отдельно |
...
| `core/usage_export.py` | Application-service | Журнал прогонов в объявленном формате для соседних инструментов: сбор событий, JSONL-рендер, запись файла со схемой |
...
| `core/submission.py` | Application-service | Общий путь отправки решения на Stepik: сеть (`core/stepik_client`) плюс запись в локальную историю, чтобы веб и CLI отправляли одинаково |
...
| `core/task_scaffold.py` | Domain (leaf) | Каталог задачи без Stepik (`--init-task`): решение плюс `tests/1`/`tests/1.clue` по открытому контракту [configuration.md](../use/configuration.md). Шаблон решения рабочий — созданная задача проходит `--mode 1` без правок; существующий каталог не перезатирается (`FileExistsError`) |
...
| `build_info.py` (top-level) | Infrastructure / Utilities (leaf) | `read_build_info()` / `display_version()` — логическая версия проекта (`1.10.234`, у релиза `1.11`) из `_build_info.json`, который кладётся в пакет при сборке (`scripts/generate_build_info.py`). Считать её в рантайме нельзя: `scripts/version.py` берёт число из git-истории, а в пакете из pipx её нет вовсе, а git-подпроцесс при старте окна подвисает на macOS + 3.14 на десятки секунд. Stdlib-only, project-импортов нет; файла нет — функции отдают `None`, и окно просто обходится без строки |
...
| `core/html_to_markdown.py` | Domain (leaf) | `html_to_markdown(html, local_files)` — условие задачи из HTML в читаемый Markdown: заголовки, списки, `pre`/`code`, таблицы примеров, ссылки и картинки; ссылка на приехавшее вложение переписывается на имя файла рядом. Соседний `task_page_parser` достаёт из того же HTML **данные**, этот — **текст для человека**; общего состояния нет. Полный HTML не поддерживается намеренно — неизвестный тег разворачивается в своё содержимое. Только stdlib (`html.parser`, `re`) |
| `core/task_index.py` | Domain (leaf) | `build_task_index(root)` / `index_signature(root)` — дерево скачанных задач курс → секция → урок → шаг. **Порядок из иерархии `meta.json`, а не из файловой системы**: алфавит каталогов ставит `task10` перед `task9`, а папки ещё и переименовывают. Подпись для кеша считает и число задач, и свежий mtime — удаление задачи максимум не двигает. Только stdlib |
...
| `rules/models.py` | Domain (leaf) | Датаклассы правила стиля и его примеров — форма записи, без чтения и без вывода |
| `rules/json_provider.py` | Domain | Чтение правил стиля из JSON-базы пакета: разбор, кэш по mtime, отбор по идентификаторам; сеть и диск задачи не трогает |
...
downloader.py          ──→  core/html_to_markdown.py  (task.html сырым, task.md конвертацией; ссылки на приехавшие вложения — на локальные имена)
...
core/task_page_parser.py / core/tests_writer.py / core/step_content.py / core/html_to_markdown.py / core/html_sanitizer.py / core/task_index.py  ──→  (ничего в проекте; чистые leaf, только stdlib)
...
launcher.py                                          ──→  build_info.py  (логическая версия для строки под заголовком; без git и подпроцессов)
...
core/runner.py        ──→  core/spawn.py  (запуск решения со страховкой от зависания в Popen.__init__; ребро внутри core/, spawn — stdlib-leaf)
