...
### Added
- Web: фильтры глоссария — кнопки семейств «Модули» (все разделы «Модуль X») и «Типы данных» поверх новой грани `GET /api/glossary?group=`, порядок контролов по частоте использования (семейства → чипы разделов → раздел/тип/сортировка/статус), сортировка `sort=relevance` (точное совпадение id/title выше упоминания в тексте) и охват поиска на `syntax`/`examples` (#685)
