...
- **Канал обратной связи (эпик #751, #752–#754):** пункт `9` интерактивного меню
  ...
  Пользовательские шаблоны переведены на YAML Issue Forms и дополнены типами
  «Идея» и «Задача Stepik проверяется неправильно» (внутренний `feature_task.md`
  остался Markdown-постановкой); `config.yml` уводит вопросы в Discussions.
...
- **Полировка веба (#633–#637, #658–#661, #670, #683–#686, #723–#727, #729,
  #731):** стартовый онбординг, тосты, посимвольный diff при WA, GUI-лаунчер
  `stepik-grader-gui` без командной строки, `RU|EN` в topbar, навигация
  глоссария семействами разделов, панель «Доступ к Stepik» в загрузчике,
  отправка решения на Stepik из режима 1, диаграмма сравнения решений в режимах
  3/4 и пре-флайт корректности перед замером скорости, focus-trap/Escape в
  модалках, перевод заголовков таблиц.
- **CLI и витрина (#601, #643–#645, #728):** скачивание задачи со Stepik прямо
  из меню, пустые состояния-тупики заменены на next-step, positive-nudge после
  серии успехов; README переписан вокруг «зачем это, если Stepik и так
  проверяет» и получил hero-GIF со скриншотами; правила PEP вызываются набором
  ровно по карточкам и с `--preview` — 17 из 36 карточек были недостижимы.
- **Периметр безопасности (#627–#632, #648, #691–#693):** fuzz входного тракта
  ...
  лимит вывода решения на **всех** путях исполнения (включая пошаговый трейс и
  микробенч), сворачивание домашнего пути в обратной связи независимо от
  регистра и 8.3-написания, consent перед отправкой
  кода AI-провайдеру распространён на CLI (в вебе гейт стоит с 1.9.0 — раньше
  CLI слал код молча), атомарная запись `secrets.json`, чистка
...
- **Документация и её гейты (#700, #757–#761):** `docs/` разложена по читателю
  ...
  незакрытые аудиты; docs-гейтов стало шесть вместо четырёх — к бюджету README,
  проверке ссылок, полноте индексов и бюджету версий CHANGELOG добавлены
  проверка направлений `docs/` и запрет журнала работ (`#NNN`) в объясняющих
  документах; `CHECKPOINT.md` удалён как дубль CHANGELOG, CONTRIBUTING сжат
  405 → 326 строк.

Метрики: 1600+ → 2295 собранных тестов (2266 прошли, 29 пропущены на Windows),
покрытие 94.8% (кросс-OS combined) / 92.6% (ubuntu). Квалити-гейты зелёные;
CI-матрица (3 ОС × 3.12/3.13 блокирующие + 3.14 экспериментальные на всех трёх
ОС, `continue-on-error`) зелёная.
