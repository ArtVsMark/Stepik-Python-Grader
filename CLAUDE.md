...
1. **Проверить очередь** (`gh_rest.py queue`). Не голова — ждать неподвижно;
   `check_pr_ready.py` тоже отвечает «ждать» и называет, кто впереди.
2. **Обновить ветку из `main`** — только голове очереди. Устаревшая ветка
   проверена на состоянии, которого уже нет.
3. **Включить auto-merge** — тоже только у головы, а не у всех сразу: PR
   смержится сам, как только пройдут обязательные проверки. Именно это снимает
   нужду опрашивать статусы — а опрос и есть то, что дважды за день выжигало
   GraphQL до нуля.
