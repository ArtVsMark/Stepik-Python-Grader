...
- **Состояние находки здесь не хранится — его читают в задаче.** Раньше на этом месте стояло обратное
  («открыта, если ID не значится в § 5-квater»), и контракт называл источником состояния сразу два места:
  трекер и реестр этого документа. Цена измерена — 493 находки из 883 разошлись между ними, а очередь мержа
  сериализовалась на общем файле: семь изменений подряд дописывали строку в одно место, и каждое
  смерженное делало следующее конфликтным. Конфликтный PR при этом остаётся вовсе без проверок
  (issue #1438, находка `AUD-3-01` из #962).

  Галочка чек-листа в задаче называет исход — номер PR или причину отклонения. **§ 5-квater реестром
  состояния не является:** он собирается один раз, при архивации, из закрывающих изменений;
  `scripts/check_audit_registry.py` называет то, что в него войдёт, а не отставание, которое надо чинить
  руками. Когда закрыты или отклонены все находки, документ целиком переезжает
...
Состояние находок ниже читают в задачах, как и в § 5, — здесь оно не хранится.
