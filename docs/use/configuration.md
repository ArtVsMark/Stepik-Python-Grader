...
| `glossary_store` | `str \| None` | `None` | Путь к локальной JSON-базе карточек глоссария (issue #126); `None` — веб-слой откатывается на компактный `core/glossary.py`. См. [glossary.md](../dev/glossary.md). |
| `glossary_missing_queue` | `str` | `".grader_glossary_missing.db"` | Путь к очереди пополнения глоссария (J7 — недостающие карточки): SQLite/WAL с issue #552; legacy `.json`-очередь читается и разово импортируется при первой записи. См. [glossary.md](../dev/glossary.md). |
