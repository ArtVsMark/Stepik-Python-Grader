...
- **Microbench: локальный per-call таймаут отсутствует** — решение, зависающее
  ...
  (issue #47 R-01).
- **`--sandbox` неприменим к `--serve` (issue #351).** Опциональный
  `SandboxRunner` (`--sandbox`, issue #266) изолирует только CLI-исполнение
  (режимы 1–4); в web-слой он не проброшен, поэтому web-сервер всегда исполняет
  код обычным `LocalRunner`'ом без изоляции. Чтобы явный флаг безопасности не
  игнорировался молча, `stepik-grader --serve --sandbox` завершается ошибкой, а
  не запускает незащищённый сервер. Проброс sandbox в web — отдельная задача
  (вне issue #266).
