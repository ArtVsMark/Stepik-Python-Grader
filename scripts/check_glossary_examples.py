...
#:
#: 90 → 67 (issue #1454): двадцать три карточки с ОДНИМ открывателем блока
#: разобраны руками. Автоматом это не делается, и попытка измерена: эвристика
#: «после двоеточия уровень +1» из блока никогда не выходит, поэтому она
#: сворачивает весь пример в лестницу вложенности — а `ast.parse` такой результат
#: ПРИНИМАЕТ, и прогон тоже (определения не исполняются). Приёмка вышла слабее
#: чинимого дефекта: сломанный пример виден, «починенный» — нет.
BUDGET = 67
