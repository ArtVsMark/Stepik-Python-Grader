...
> Архив «остывших» записей `CHANGELOG.md` — две части:
>
> 1. **Версионированные релизы** (`## [X.Y.0] - ДАТА`) — ротированы из живого
>    `CHANGELOG.md` (issue #373), чтобы там оставались только `[Unreleased]` +
>    три последних MINOR. Формат идентичен живой части.
> 2. **До-версионные записи** — сделаны **до** git-тегов/`setuptools-scm`
>    (issue #162/#183), датированы июнем 2026 и оформлены как
>    `## [unreleased] / <дата>`, а не как релизы `## [X.Y.0] - ДАТА`; вынесены
>    сюда issue #267.
>
> Живая часть (актуальный `[Unreleased]` + три последних MINOR) остаётся в
> [`../CHANGELOG.md`](../CHANGELOG.md). Тот же период по-русски, сводно, на
> уровне спринтов/issue — в [history.md](history.md): здесь построчный
> английский лог по PR, там — курируемая русская сводка (разные уровни
> детализации, не рассинхронизированный дубль).

---

## Архив версионированных релизов (1.1.0 – 1.5.0)
...
## До-версионные записи (июнь 2026)
