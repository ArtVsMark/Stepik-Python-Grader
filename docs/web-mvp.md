...
> PR-6). Изначально дизайн-документ; **#125 (workspace проверки решений) —
> реализован** (split-pane, ErrorCard, action cards, palette, сценарные
> кнопки, раздел «Глоссарий») — см. `src/stepik_grader/web/`. **#186
> (Downloader в web) — реализован**, см. ниже § «Downloader / Загрузка
> тестов» (важное отличие от изначального дизайна: раздел «Загрузчик задач»
...
> фидбеку в #125.** Владелец приложил к #123 эталонный макет
> (`web-mvp-mask.html`) и уточнённый layout-каркас (Topbar/Sidebar/Main/
> Insight pane). Визуальный слой (`static/{index.html,app.css,app.js}`)
> переписан под design-токены и grid-based `.app-shell` макета — вся
> функциональность #125 сохранена, только новая разметка/классы.
>
> Первая итерация редизайна ошибочно трактовала «Режим 1» макета как
...
> run_again/toggle_theme/switch_section, когда ничего не выбрано) убраны из
> приложения целиком; command palette и action cards в detail panel не
> изменились.
>
...
> `core/microbench_runner.apply_relative_ranking` не тронут) остаётся в коде
> и покрыта тестами, но фронтенд её не вызывает ни в одном режиме — пригодится
> позже, когда эталон из #55 понадобится сравнивать с bench/microbench.
>
...
>    symmetричный «Проверке решений»/«Глоссарию») — типичный путь новичка:
>    зайти в раздел, вставить URL, скачать, кнопкой «Перейти к проверке»
>    подставить путь в раздел «Проверка решений» (J0).
...
>
> Изначальный дизайн этого документа планировал Downloader как workflow-блок
> внутри «Проверки решений», без отдельной вкладки topbar/sidebar — при
> реализации (#186) владелец пересмотрел это решение в пользу полноценного
> отдельного раздела, используя уже существующий sidebar-пункт; см.
...
«Загрузчик задач», «Глоссарий». Изначальный дизайн планировал Downloader как
workflow-блок внутри «Проверки решений» (см. врезку в начале документа) —
при реализации (#186) владелец подтвердил отдельный, полноценный раздел,
используя уже существующий sidebar-пункт.
...
MVP, реализован в issue #186 как отдельный раздел «Загрузчик задач». CLI-путь
...
OAuth-авторизацию — веб-раздел лишь показывает понятную ошибку, если валидного
токена нет (см. § «Endpoint'ы» ниже).
...
не найдено — те же 4 приоритета, что в `downloader.py`, теперь пробрасываемые
наружу из `save_task_files`/`process_step_url`).
...
логики скачивания в web не появляется, DAG без циклов сохраняется (`web →
downloader → core`). Единственные core-правки — аддитивные: `save_task_files`/
`process_step_url` теперь возвращают уже вычисляемые внутри `(count, source)`/
`task_dir` вместо `None`; `core/oauth_flow.try_create_session_without_browser`
— новая функция, не меняющая `create_user_session`.
...
`refresh_token` (оба безопасны — не блокируют поток), а если ни один путь не
сработал — веб-раздел показывает понятную ошибку «нужна авторизация» со
ссылкой на `installation.md`.
...
решений/ошибок, детектор — J7 ниже) и *source-driven* (из будущего сканирования
покрытия stdlib). Роли и источники канонично сведены в
...
- **Downloader-адаптер** (✅ реализован, #186) — `web/downloader_adapter.py`,
  тонкая web-обёртка над `downloader.py::process_step_url` (сам вызывает
  `parse_stepik_step_url`/`build_task_directory`/`save_task_files` внутри) +
...
| `POST /api/glossary/export` | статус экспорта во внешний глоссарий | adapters → exporter (#126-follow-up) |
| ✅ `GET /api/commands?context=` | `CommandAction[]` (реестр, отфильтр. по тегам) | `web/commands.py` |
...
> (sidebar/result/detail), расширенные ErrorCard-поля (WA/RE/TLE), 5 MVP
> action cards, command palette, сценарные кнопки и раздел «Глоссарий»
> (поиск/карточка/backlog очереди пополнения) реализованы в
> `src/stepik_grader/web/` (пакет, эволюция бывшего `web.py`). Детали и
> сознательные упрощения (фиксированный словарь тегов вместо predicate-DSL,
> substring вместо fuzzy-поиска, TLE без `glossary_ids`) — в истории PR/
> CHANGELOG. `create_test`/`compare_solutions`, hash-роутинг и полный
> a11y-аудит остаются **later**, как и было спроектировано.

> **#186 (Downloader в web) — закрыт.** `web/downloader_adapter.py::download_task`
> реализован как отдельный раздел «Загрузчик задач» (не workflow-блок, как
> изначально спроектировано в этом документе, — владелец пересмотрел решение
> при реализации). OAuth — только `try_create_session_without_browser`
> (валидный токен/refresh, без похода в браузер); первичная авторизация
> остаётся за CLI. `source`/`format`/`count` в `TestCaseSet` — `source` из
> аддитивного возврата `save_task_files`/`process_step_url` (не восстановим
...
> ([glossary.md](glossary.md)); web-раздел построен поверх них в #125,
