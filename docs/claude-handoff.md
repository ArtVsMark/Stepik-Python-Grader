# Claude handoff — план работ и архив постановок

> **Статус (2026-07-15): активный план + архив.** Раздел «План работ по открытым
> issue» ниже задаёт порядок волн и зависимости по актуальному бэклогу (открытые
> задачи есть). Постановки под ним **закрыты и реализованы** — сохранены как архив
> scope/non-goals реализованных фич. Актуальные статусы — `gh issue list`.
>
...
## План работ по открытым issue — 2026-07-15 (активно)

> **Статус: активный план.** Составлен по всем открытым issue (на 2026-07-15 — 61:
> 6 эпиков + 55 листовых). **Волна 1 выполнена — PR #448; Волна 2 выполнена —
> PR #450/#451/#452**, осталось 46 открытых. Текущая точка входа — **Волна 3**.
> Порядок — по риску для пользователя **сейчас**, а не по амбициям v2.0.
...
- Глоссарий-грайнд: эпик **#363** (832 draft→ready, волны В1–В6), **#371** (пилот
  В1), **#438** (LLM-конвейер — ускоряет, не заменяет ручное ревью).
- Server mode: эпик **#151** → дизайн-задачи **#153/#154/#155** (дизайн закрыт,
  реализация не начата). **#59** (roadmap: Docker-sandbox, платформы, дашборд —
  AI-ветку конкретизируют #434/#435). **#55** (импорт закреплённого решения Stepik).
