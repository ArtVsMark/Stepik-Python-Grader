...
### Added
- core: клиент отправки решения на Stepik — attempt → submission → poll вердикта (`create_attempt`/`submit_solution`/`poll_submission`/`submit_and_wait`) + `read_step_id` из meta.json (`core/stepik_client.py`); авторизация Bearer-токеном сессии (CSRF под Bearer не нужен). Фундамент кнопки «Отправить в Stepik» в режиме 1 — web/UI отдельным PR (#683)
