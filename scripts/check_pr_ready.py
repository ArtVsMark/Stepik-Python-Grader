...
def metadata_blockers(pull: dict[str, Any]) -> list[str]:
    ...
    # Правило 173 каталога: у изменения РОВНО ТРИ допустимых ответа — закрывает
    # задачу целиком, делает часть и называет остаток, либо освобождено строкой
    # с причиной. Четвёртого («слить и промолчать») нет.
    #
    # Второго ответа здесь долго не было, и это не мелочь формы: у частичной
    # работы по заведённой задаче честной формы не существовало вовсе. `Closes`
    # закрыл бы задачу целиком, «Без issue» — неправда, задача есть. Развилку
    # приходилось обходить заведением подзадачи (PR #1415 → issue #1416):
    # приём законный, но выбор между ним и ложью в теле решался памятью окна.
    #
    # Асимметрия, из-за которой «частично» — отдельный ответ, а не округление:
    # незакрытая сделанная задача держит очередь и берётся в работу заново,
    # ложно закрытая — хуже, остаток исчезает вместе с ней (issue #1419).
    if not (_CLOSES_RE.search(body) or _PARTIAL_RE.search(body) or _NO_ISSUE_RE.search(body)):
        reasons.append(
            "в теле нет ни «Closes #N», ни «Часть #N — <что именно сделано>», "
            "ни строки «Без issue: <причина>» — без связи задача не закроется "
            "при мерже, а приоритет очереди брать неоткуда"
        )
    return reasons
