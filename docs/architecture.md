...
| `web/server.py` | Application / Web | HTTP-хендлер (stdlib `http.server`, `--serve`): декларативные таблицы маршрутов REST-API (`_API_GET/POST_EXACT/PREFIX`) — **полный перечень эндпоинтов см. [api.md](api.md)** (канон, защищён контрактным тестом `test_web_api_contract.py`); статика (`static/` — `index.html`/`app.css`/`app.js` + `fonts/` + `vendor/codemirror-bundle@6.mjs`) читается при импорте; тонкий слой поверх `viewmodels.py`/адаптеров ниже, бизнес-логики не добавляет |
