...
### Internal
- Прогонный корпус — `scripts/corpus_run.py` + `scripts/corpus_mutations.py` + `corpus/`: сверка вердикта ядра с ожидаемым на задачах целиком, 12 детерминированных мутаций эталона (ловят ложный AC и ложный WA), шесть посевных задач со своими условиями, разбор и границы — `docs/dev/corpus.md`
