...
веб-интерфейс: страница с полем пути → таблица результатов с AC/WA, временем,
памятью и diff при WA (клик по имени файла раскрывает тест-кейсы).
...
- **Без новых зависимостей** — на stdlib `http.server`, переиспользует ту же
  логику грейдинга, что и CLI (`run_tests`).
- В поле пути — файл решения (`.py`) или папка с решениями; тесты
  резолвятся так же, как в режимах 1/2.
