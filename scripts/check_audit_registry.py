#!/usr/bin/env python3
"""scripts/check_audit_registry.py — что войдёт в реестр находок при архивации.

**Состоянием реестр не является — им является задача** (issue #1438). Прежняя
редакция этого файла утверждала обратное: «находка открыта, если её ID не
значится в реестре». Контракт при этом называл источником состояния и трекер, и
реестр, каждый раз объявляя исключительность, и держал эта проверка ровно ту
половину, которую соседний раздел запрещал. Цена: 493 находки из 883 разошлись
между колонкой состояния и реестром одного документа, а очередь мержа
сериализовалась на общем файле — семь изменений подряд дописывали строку в одно
место, и каждое смерженное делало следующее конфликтным.

Предмет проверки поэтому другой: она называет, **что войдёт в реестр при
архивации**, а не отставание, которое надо чинить руками. Реестр собирается один
раз, в конце жизни документа; ведёт состояние чек-лист задачи.

Замер, из которого проверка выросла, остаётся верным и объясняет, зачем она
нужна вообще: 26.08.2026 реестр аудита 2026-08-10 содержал 57 записей при 209
фактически закрытых. Без списка «что закрыто» собрать реестр при архивации не из
чего — его пришлось бы восстанавливать чтением всей истории мержей.
