...
> 6 эпиков + 55 листовых). Порядок — по риску для пользователя **сейчас**, а не по
> амбициям v2.0. Критические пункты сверены с фактическим кодом (не с текстом
> issue) — план опирается на реальные блокеры. Критерии приёмки — в самих GitHub
> issue; здесь — «что за чем и почему».
...
**Волна 1 — «Стоп-краш/хэнг» (v1.8.x, немедленно).** Подтверждённые репро: краши,
