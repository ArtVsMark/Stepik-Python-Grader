...
десять принимались одинаково — по диффу, без запуска. Отсюда три требования.
...
результат до и после. Если поверхность недоступна (нужен реальный токен Stepik
или графическая оболочка) — скажите это прямо в PR и опишите, что именно
осталось непроверенным; см.
[`docs/agent/claude-handoff.md § Где выполнять`](docs/agent/claude-handoff.md).
