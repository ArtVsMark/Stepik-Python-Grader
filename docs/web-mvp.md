...
>
> **Редизайн под маску эпика #123 — выполнен.** Владелец приложил к #123
> эталонный макет (`web-mvp-mask.html`) и уточнённый layout-каркас (Topbar/
> Sidebar/Main/Insight pane). Визуальный слой (`static/{index.html,app.css,
> app.js}`) переписан под design-токены и grid-based `.app-shell` макета —
> вся функциональность #125 сохранена, только новая разметка/классы. Заодно
> добавлен режим **«Сравнение» (Compare, режим 1 в mode-row)**:
> `grade_benchmark(reference=...)` ранжирует относительно указанного
> локального файла-эталона (вердикты `REFERENCE`/`FASTER` — новые, поверх
> существующих `SIMILAR`/`SLOWER`/`MUCH_SLOWER`), не изменяя
> `apply_relative_ranking` (общий с CLI). Это не тот же механизм, что issue
> #55 (переоткрыт, полуавтоматический импорт эталона со Stepik JSON) —
> `#ref-input` принимает только уже локальный файл, ручным вводом.
>
