...
### Added
- Web: «Функции в коде» ловит исключения, названные в решении (`raise ValueError`, `except KeyError:`, `RuntimeError("…")`), атрибуты импортированных модулей без вызова (`math.pi`, `sys.argv`) и методы stdlib-классов (`Path.exists()`); имена методов, которых нет в stdlib-инвентаре, берутся из самой базы глоссария (#686)
