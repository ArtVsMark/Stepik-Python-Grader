...
В пакете лежит готовая база — `src/stepik_grader/glossary/data/*.json` (601
карточка на момент правки — пополняется волнами наполнения глоссария, эпик
#363; по файлу на цветовую группу `cg`), частично импортированная из внешнего
