...
- `grader.py` (~1400 строк) совмещает логику запуска, форматирования и CLI.
  При росте проекта планируется выделить `cli.py` и `reporter.py`.
- Отсутствует `src/`-layout — возможны конфликты импортов при установке как пакета.
