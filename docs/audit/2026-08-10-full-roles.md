...
| SBX-5-01 | Кэш отдаёт обычному прогону вердикт, порождённый изоляцией | #1010 |
| CNC-5-05 | --sandbox вместе с --cache не заходит в песочницу вовсе | #1010 |
| PY-3-01 | Ключ кэша не учитывает условия исполнения | #1010 |
| PERF-1-01 | Устаревший вердикт при смене timeout_seconds/--sandbox | #1010 |
| PY-3-02 | cache.save() роняет грейдинг OSError вопреки инварианту | #1010 |
