...
**Мержим через merge queue, а не кнопкой.** PR встаёт в очередь, GitHub сам
собирает ветку «`main` + этот PR», гоняет на ней полный CI и мержит только при
зелёном. Три вещи это закрывает разом: прогоны перестают отменять друг друга
(у кандидата свой ref, вытеснять нечем — раньше шесть мержей подряд дали шесть
`cancelled` и ни одного выполненного), «не мержить до полного CI» выполняется
буквально и **до** попадания в `main`, а ожидание не стоит ни одного запроса
нашим токеном — очередь ждёт внутри GitHub. Бонусом ловится **семантический
конфликт**: два PR, зелёных по отдельности, но ломающих друг друга вместе,
проверяются на объединённом состоянии.

Ловушка настройки одна и тихая: **без триггера `merge_group` в `ci.yml`
проверки на кандидате не запускаются вовсе**, и очередь мержит вслепую — в
интерфейсе при этом всё выглядит включённым.

Пока очередь не включена в защите ветки, страхует `check_pr_ready.py`: он
говорит «ждать», пока на `main` идёт прогон, и «нельзя», если последний прогон
`main` красный. После включения очереди проверка становится избыточной, но не
вредной — один дешёвый REST-запрос.
