...
### Added
- Web: навигация по глоссарию — шесть раскрываемых семейств разделов (Модули 707 · Типы данных 311 · Синтаксис · Встроенные и исключения · Ввод-вывод и файлы · Алгоритмы) поверх новой грани `GET /api/glossary?group=`; клик по семейству фильтрует выдачу и раскрывает его разделы со счётчиками. Семейства покрывают все 52 раздела, поэтому селект «Раздел» и дублирующие чипы убраны, а селект «Статус» показывается, только если в источнике есть черновики (в комплектной базе их нет). Добавлена сортировка `sort=relevance` (точное имя выше упоминания в тексте, `str.split` выше `bytearray.split`) и охват поиска на `syntax`/`examples` (#685)
