...
- Глоссарий: расширенное описание `body` (RU+EN) ещё у 160 карточек — раздел «Исключения» и начало «Строк»; поле выросло с 21% до 33%. Пишется только там, где добавляет неочевидное поверх `summary`: `FileNotFoundError` чаще про относительный путь от рабочей директории, чем про отсутствующий файл; `GeneratorExit` наследует `BaseException`, чтобы `except Exception` не мешал закрытию; `len` у строк считает кодовые точки, а не видимые знаки (#722)
