...
- 📚 Локальный глоссарий-модуль (581 карточка): функции/исключения/конструкции,
...
(`core/sandbox/`, три backend'а, issue #266); по умолчанию выключена и в веб-слой
(`--serve`) пока не проброшена (issue #351). Без `--sandbox` запускай только
доверенные решения (свои или скачанные из Stepik as-is).
