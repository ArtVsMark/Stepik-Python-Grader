...
Переключатель режимов в command bar — сегмент-контрол **Корректность /
Бенчмарк / Микро-бенчмарк**. Микро-бенчмарк (режим 4) меряет время через
`timeit` внутри процесса и показывает колонки, отличные от режима 3:
...
- **Topbar**: логотип, переключатель разделов (сегмент-контрол), тумблер
  темы, кнопка Command palette (`⌘K` / `Ctrl+K`).
...
(тест-кейс / error card) inline. Deep-linking состояния через URL-хэш — пока
не реализован, см.
