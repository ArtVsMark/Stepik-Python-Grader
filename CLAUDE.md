...
**PR, открытый вручную, помечается `merge-when-green`** — метка включает ему
авто-мерж (см. § Гейты и `CONTRIBUTING.md`). Это запасной путь для того, что
уже открыто: заводить ветку заново ради префикса дороже, чем поставить метку.
