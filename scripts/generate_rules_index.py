...
#: Статусы, которыми проект отвечает «здесь это не действует». Оба несут `why`
#: по контракту каталога, то есть решение принято и записано.
_ANSWERED_ELSEWHERE = ("rejected", "not-applicable")


def _answered_elsewhere(path: pathlib.Path | None = None) -> set[str]:
    """Номера правил, про которые проект уже ответил «здесь не действует».

    Такое правило в указатель не идёт, и это не потеря: причина обязательна по
    контракту каталога и лежит в `.rules/bindings.json` рядом со статусом.

    Без этого отбора отвеченное сливалось с неотвеченным. Статус, отличный от
    `active`, не попадал в словарь уровней, и вызывающая сторона показывала
    «не объявлено» — ту самую метрику, которая объявлена очередью на
    автоматизацию и обязана уменьшаться. Правило 079 (`not-applicable`,
    причина названа) стояло в ней и уменьшить её было нечем: автоматизировать
    предмет, которого здесь нет, невозможно. Два разных состояния —
    «решено, что не про нас» и «руки не дошли» — читались одинаково, а стоят
    они по-разному (правило 154).
    """
    target = path if path is not None else _ROOT / ".rules" / "bindings.json"
    if not target.exists():
        return set()
    try:
        ...
        # Битый ответ — не повод прятать правило: пусть остаётся в указателе.
        return set()
    return {
        str(rule_id)
        for rule_id, raw in (data.get("rules") or {}).items()
        if isinstance(raw, dict) and raw.get("status") in _ANSWERED_ELSEWHERE
    }
...
#: Передача владения ВНУТРИ абзаца. Владельца задаёт первый названный
#: репозиторий, но след правила, родившегося здесь, часто заканчивается фразой
#: «а у каталога это вот чем» — и путь после неё принадлежит уже не нам.
#: Прецедент: правило 181 вышло отсюда, его след начинается нашим репозиторием и
#: заканчивается «У каталога — `scripts/check_exclusive.py`». Указатель считал
#: этот путь своим, не находил файла и отказывался пересобираться целиком —
#: гейт, краснеющий на верном ответе.
_HANDOVER_RE = re.compile(r"У\s+(?:каталога|соседа|них)\b")
...
def _our_paths(trace: str) -> list[str]:
    ...
    Владение передаётся и ВНУТРИ абзаца: след правила, родившегося здесь,
    часто кончается фразой «а у каталога это вот чем», и путь после неё уже не
    наш. Пути после такой передачи не берутся.
    ...
    то есть отказывался пересобираться из-за чужого механизма. Правило 181
    добавило второй случай: оно вышло отсюда, поэтому абзац наш, а последняя
    фраза — про каталог.
    """
    found: list[str] = []
...
def collect_rules(catalogue: pathlib.Path, *, repo_root: pathlib.Path | None = None) -> list[Rule]:
    ...
    held = _bindings_mechanisms(bindings)
    elsewhere = _answered_elsewhere(bindings)
    collected: list[Rule] = []
    broken: list[str] = []
    for path in files:
        ...
        # След сюда ведёт и предмет на месте, но проект ответил «здесь этого
        # нет» либо «решение иное» — и назвал причину. Указатель перечисляет
        # действующее, поэтому такое правило пропускается, а не показывается
        # необеспеченным. Проверка следа идёт РАНЬШЕ пропуска: ответ
        # «не про нас» не повод не заметить, что предмет исчез.
        if rule.number in elsewhere:
            continue
