...
| `core/history.py` | Infrastructure / Utilities | Opt-in SQLite-история прогонов: `record_run`/`read_recent_runs`, база `.grader_history.db` (runs/case_results/lint_violations), WAL + `user_version`-миграции, best-effort. Фундамент разделов «Правила»/«Подучить» |
...
| `core/progress_export.py` | Application-service | Экспорт прогресса в Markdown/HTML (`--export-progress`) над `core/history`/`core/insights`: `build_progress_report`/`render_markdown`/`render_html` — per-task TTFG и tallies решённого, без сети |
