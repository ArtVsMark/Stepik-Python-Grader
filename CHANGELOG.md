...
### Fixed
- Тесты: смок-тест аудита примеров глоссария стал ratchet'ом — раньше он исполнял примеры всех 1349 карточек и заканчивался ассертом `flagged >= 0`, истинным всегда, поэтому CI платил десятки секунд и не ловил ни одного битого примера (при `flagged=389`). Теперь известный долг перечислен поимённо в `tests/glossary_audit_known_issues.txt`, и карточка, которой в списке нет, валит прогон; список ведётся по карточкам, а не числом, потому что набор расхождений зависит от ОС и версии Python (#746)
