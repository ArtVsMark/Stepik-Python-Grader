...
def test_python_cmd_is_sys_executable() -> None:
    """_PYTHON_CMD == sys.executable, а не платформо-зависимая строка.

    На Windows "python"/"python3" может указать на системный Python вне venv;
    sys.executable всегда указывает на интерпретатор, запустивший процесс.
    """
    assert executor._PYTHON_CMD == sys.executable
