...
в этом тесте — **отвергающий** случай: утверждение, ожидающее непустой список
находок либо ненулевой код возврата. Односторонний набор (только «на живом
репозитории чисто») ловит ровно ту ошибку, которой не бывает: он зеленеет и
тогда, когда проверка сломана и не находит ничего никогда (правило 075).
...
__all__ = ["KNOWN_DEBT", "REJECTION_MARKERS", "gates_without_rejection", "main", "tests_for"]
...
def gates_without_rejection(
    ...
    for gate in gates:
        ...
        if not related:
            ...
            continue
        if not any(marker in tests[name] for name in related for marker in REJECTION_MARKERS):
            problems.append(
                (
                    gate,
