...
_LEAF_MODULES = (
    ...
    # ``build_info`` (issue #1262) — сведения о сборке из `_build_info.json`.
    # Leaf по необходимости, а не по вкусу: его читает окно лаунчера при старте,
    # и любой проектный импорт здесь означал бы лишнюю работу на пути к первому
    # кадру — а git-подпроцесс на macOS + 3.14 стоил бы десятков секунд
    # (#1166/#1149), ради чего файл и кладётся при сборке.
    "stepik_grader.build_info",
    ...
    # ``core.html_to_markdown`` (issue #1176) — конвертер условия задачи в
    # Markdown. Leaf по замыслу: разбирает строку в строку, поэтому проверяется
    # без моков сети и ФС. Соседний ``core/task_page_parser`` достаёт из того же
    # HTML данные (тест-кейсы, ссылки) — общего состояния у них нет.
    "stepik_grader.core.html_to_markdown",
    # ``core.html_sanitizer`` (issue #1177) — очистка условия перед показом.
    # Защитный код обязан быть leaf: чем меньше он тянет, тем меньше поверхность,
    # через которую в него попадёт чужое поведение.
    "stepik_grader.core.html_sanitizer",
    # ``core.task_index`` (issue #1179) — обход дерева скачанных задач.
    # Leaf: разбирает диск в структуры, статусы приклеивает адаптер.
    "stepik_grader.core.task_index",
)
