...
### Security
- Вывод решения на пути с отменой (веб-прогоны, песочница) больше не копится в памяти без границы: новый `CONFIG.max_output_bytes` (10 МБ) ограничивает **накопление** stdout+stderr, лишнее отбрасывается с пометкой в stderr, а дренаж продолжается — процесс не зависает на переполненном pipe и доживает до своего таймаута. Раньше лимит был только у `SandboxRunner`, и решение с бесконечным `print` набивало RAM хоста, а пул параллельных job'ов клал процесс по OOM (#629, частично)
