...
### Fixed
- Тесты: смок-тест аудита примеров глоссария стал ratchet'ом — раньше он исполнял примеры всех 1349 карточек и заканчивался ассертом `flagged >= 0`, истинным всегда, поэтому CI платил десятки секунд и не ловил ни одного битого примера (при `flagged=389`). Теперь число «требующих внимания» ограничено потолком: карточка с битым примером валит прогон, а разбор накопленного идёт отдельно (#746)
