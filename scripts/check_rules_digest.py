...
def check_hook_is_registered(errors: list[str], settings: str | None = None) -> None:
    ...
    for event, script in _REQUIRED_HOOKS.items():
        ...
        # вышло: `.gitignore` игнорировал `.claude/*` целиком, хуки жили только
        # в рабочей копии — в чистом клоне и в CI механизма не существовало, а
        # настройки при этом честно его объявляли.
        if not (_ROOT / ".claude" / "hooks" / script).exists():
            errors.append(
                f"хук {event} объявлен, но .claude/hooks/{script} нет в репозитории — "
