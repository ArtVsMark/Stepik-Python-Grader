...
схема ``1.2``; заготовка — `templates/bindings.json` там же.
...
4. **Каждый номер версии называет свой предмет** (правило 164). Ключ
   ...
   потребителя (1.2), предложения (1.0) и сводки потребителей (1.0). Сам ключ
...
#: Версия ВЫГРУЗКИ каталога, по которой построены ответы (контракт 1.2). Два
#: номера двигаются порознь: формат ответа versus формат того, что мы читаем.
#: Перечитывать ответы заставляет подъём второго, и до появления этого ключа
#: сверять его было не с чем.
ANSWERS_KEY = "answers_to"
...
#: двигаются независимо: ответ — 1.2, предложение — 1.0. Один ключ на оба и
#: есть та ошибка, ради которой заведён `SUBJECT_KEY`.
#:
#: Константа здесь неизбежна: формат проверяется на КАЖДОМ изменении, а клона
...
#: Четыре уровня контракта 1.2. Граница между ними — один вопрос: что
...
def binding_violations(data: dict[str, Any], *, root: Path | None = None) -> list[str]:
    ...
    # Контракт 1.2 завёл ВТОРОЙ номер: версию выгрузки, по которой ответы
    # построены. Перечитывать ответы заставляет подъём именно её, а не номера
    # самого ответа, — и до 1.2 этот номер не сверялся ничем. Наш собственный
    # промах и есть прецедент: прозой стояло «выгрузка — 1.2» при живой 1.5,
    # отставание на три подъёма, невидимое обеим сторонам.
    if not str(data.get(ANSWERS_KEY) or "").strip():
        problems.append(
            f"нет ключа {ANSWERS_KEY!r} — версия выгрузки, по которой построены ответы, "
            "не названа; без неё подъём читаемого контракта не с чем сравнить"
        )
...
def export_contract(catalogue: Path) -> str | None:
    """Версия выгрузки правил, объявленная самим каталогом (``None`` — не прочли).

    Спрашивается у выгрузки, а не хранится копией: два номера одного контракта
    расходятся молча, и здесь это уже случилось — проза в нашем ответе называла
    выгрузку 1.2 при живой 1.5.

    Args:
        catalogue: Клон каталога правил.

    Returns:
        Номер контракта выгрузки либо ``None``, если файл не прочитан.
    """
    export = catalogue / "export" / "rules.json"
    try:
        data = json.loads(export.read_text(encoding="utf-8"))
...
def contract_drift(data: dict[str, Any], catalogue: Path) -> list[str]:
    ...
    if published is not None and ours != published:
        problems.append(
    ...
    # Второй номер — тот, ради которого правило 157 и существует: ответы
    # строятся по ВЫГРУЗКЕ, и перечитывать их заставляет её подъём. Номер
    # самого ответа при этом может не двигаться годами.
    live = export_contract(catalogue)
    answered = str(data.get(ANSWERS_KEY) or "")
    if live is not None and answered != live:
        problems.append(
            f"выгрузка каталога — {live}, ответы построены по {answered or 'неназванной'}. "
            "Подъём ЭТОГО номера и есть повод перечитать ответы: меняется то, что мы "
            "читаем, а не то, что пишем"
        )
    return problems
...
def version_subjects(*, root: Path | None = None) -> list[str]:
    ...
    потребителя (1.2), предложение (1.0), сводка потребителей (1.0). Один ключ
