...
Каждая карточка ответа несёт поле `group` — семейство её раздела, вычисляемое
сервером (issue #685): UI строит по нему и кнопки семейств, и список разделов
внутри раскрытого, не повторяя правило у себя. Семейства покрывают **все**
разделы базы (неклассифицированный раздел падает в `other`), потому что в UI
они заменили собой селект «Раздел».
