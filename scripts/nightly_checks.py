...
CHECKS: tuple[Check, ...] = (
    ...
    Check(
        "След каталога в наше дерево разрешается",
        ["scripts/check_catalogue_trails.py", "--catalogue", _CATALOGUE_TOKEN],
        "переименование раздела проходит зелёным, а ссылка каталога обрывается молча",
    ),
    Check(
        "Экспериментальной помечена предрелизная версия",
        ["scripts/check_experimental_python.py"],
        "флаг стареет от чужого релиза: падения на вышедшей версии перестают блокировать мерж",
    ),
    Check(
    ...
    Check(
        "Судьба задачи после слияния",
        ["scripts/check_issue_state_after_merge.py"],
        "закрытая закрыта, частичная открыта и несёт остаток галочками",
    ),
    Check(
    ...
    Check(
        "Имя каталога не стало редиректом",
        ["scripts/check_catalogue_name.py"],
        "переименование чужого репозитория не ломает ничего сразу — и потому незаметно",
    ),
    Check(
        "У страховки расписанием есть замер",
        ["scripts/measure_queue_wakeups.py"],
        "расписание площадки — пожелание, а не частота; «работает» без чисел держится верой",
    ),
    Check(
        "Старое имя репозитория не осталось в дереве",
        ["scripts/check_stale_repo_names.py"],
        "редирект площадки чинит адрес за нас и потому не краснеет нигде",
    ),
    Check(
        "Каталог ответил на наши предложения",
        ["scripts/check_proposal_verdicts.py", "--catalogue", _CATALOGUE_TOKEN],
        "принятое предложение предложением быть перестало и требует ответа по номеру",
    ),
    Check(
        "У подделок чужих интерфейсов есть источник",
        ["scripts/capture_github_fixtures.py", "--check"],
        "зелёное на сочинённой подделке ничего не доказывает; образец переснимают, а не правят",
    ),
    Check(
        "Витрина предъявила замечания к содержанию",
        ["scripts/read_objections.py"],
        "издатель считает, потребитель читает; находкой считается рост, а не наличие",
    ),
    Check(
