...
|---|---|---|---|
| **Проверка решений** | Проверка решений | Прогон файла/папки на корректность (режимы 1/2), subprocess-бенчмарк (режим 3) и микро-бенчмарк timeit (режим 4); таблица вердиктов, diff при WA, error cards при RE/TLE | `grade_path()` / `grade_benchmark()` / `grade_microbench()` → `run_tests` / `run_benchmark` / `run_microbench_mode` |
