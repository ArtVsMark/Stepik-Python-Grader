# Локальный глоссарий — формат JSON и API
...
**одностороннего** экспорта готовых карточек.
...
2. **Source-driven** — из сканирования покрытия официального Python/stdlib
   (инвентаризация того, что ещё не описано):
...
### Односторонний поток, но не односторонняя связь

Содержание течёт только наружу, и это не меняется. Но у односторонности есть
обратная сторона: **витрина видит дефекты, чинить которые может только
грейдер** — она рендерит наши карточки и упирается в пример, который не
компилируется, или в пустое тело на одном из языков.

Раньше предъявить их было нечем: витрина собирала отчёт в Markdown, человек
переносил его в задачу руками, и список из четырёхсот идентификаторов
устаревал следующей же выгрузкой. Теперь работает тот же приём, каким сама
витрина читает наш `facts.json`, — **издатель считает, потребитель читает**:

| | |
|---|---|
| Что | `.github/badges/objections.json` — замечания витрины к содержанию |
| Где | ветка `badges` репозитория [Glossary-Python](https://github.com/ArtVsMark/Glossary-Python) |
| Чем читаем | `python scripts/read_objections.py` (contents-API, `?ref=badges`) |
| Кому | ночной обход `scripts/nightly_checks.py` — он и ведёт задачу-адресата |

Три вещи, ради которых читатель устроен именно так:

- **Находкой считается рост, а не наличие.** Известный долг объявлен числом в
  `BASELINE`; краснеть каждую ночь на семи сотнях замечаний значило бы приучить
  не смотреть. Правило, которого в объявлении нет, красное сразу. Пороги
  сверяются тестом со снятым образцом (`tests/fixtures/glossary/objections.json`):
  храповик, чьи числа не сходились с источником, не держит ничего.
- **Находка без `cards` не теряется.** `section-size` относится к разделу, а не
  к карточке, и у потребителя, строящего работу по списку карточек, выпадает
  молча. Она печатается своим разделом.
- **Незнакомый идентификатор называется, а не отбрасывается.** `cards[]` витрины —
  это наши `GlossaryCard.id`; расхождение означает, что стороны разъехались.

Реестра замечаний у нас нет намеренно: он был бы вторым источником рядом с
файлом витрины и устаревал бы ровно так же, как переносимый руками Markdown.

**Одно число уже сошлось независимо.** Витрина считает `example-compiles` — 90
карточек, чей пример не компилируется; ровно столько же насчитал наш
`scripts/check_glossary_examples.py`, у которого порог `BUDGET = 90`. Причина
дефекта — импортёр: `_split_examples` срезал отступ у каждой строки по
отдельности, и тело блока уезжало на нулевой уровень (`IndentationError` при
любом запуске). Импортёр исправлен — снимается **общий** отступ блока, — но
уже сохранённые карточки этим не чинятся: их правит отдельная работа, а
храповик не даёт числу расти.
...
Поля `url` (ссылка в витрину Glossary-Python) у карточки **нет**:
поток односторонний, витрина — копия этой базы, и ссылка из оригинала в копию
уводила бы студента на устаревшую карточку. Единственный адрес карточки — её
`id` как якорь своего раздела «Глоссарий» (`#/glossary/<id>`); единственная
внешняя ссылка карточки — `docs_url` на официальный `docs.python.org`. Старый
ключ `url` во входном JSON молча игнорируется `GlossaryCard.from_dict`.

Пример — [`examples/glossary.sample.json`](examples/glossary.sample.json).

## Ревизия и инварианты карточек
...
- **Matcher-safety мультифункциональных карточек** — если `title` перечисляет
  ...
  склеенный `title`) и coverage ложно считают вложенное имя непокрытым. Решение
  гибридное: часть бандлов разбита на 1-концепт-карточки, остальным добавлены `keywords`.
- **EN-ratchet** — число карточек без `summary_en` не растёт выше
  зафиксированного порога `MAX_CARDS_WITHOUT_EN`. Все 525 карточек старого
  импорта из Glossary-Python переведены, поэтому порог сведён к **0**:
  теперь это жёсткий гейт — карточка с RU-`summary` обязана нести и EN, иначе
  `localized()` молча откатит её на русский при `?lang=en`.
...
### Дубли двух поколений импорта (`X` и `X-1`)

Два поколения импорта описали 30 концептов дважды: тематическая карточка `X`
(`abs`, `int`, `@property`, `collections.deque`, …) и карточка-каталог `X-1` из
второго прохода. Пары слиты: выживает базовый `id` (`X`) вместе с более богатыми
примерами, из `X-1` перелито уникальное — точный `docs_url`, недостающие формы
сигнатуры и примеры. Правило раздела:

- **член stdlib-модуля** (`collections.deque`, `contextlib.suppress`,
  `copy.deepcopy`, `functools.partial`, `@dataclass`) живёт в разделе
  «Модуль X» рядом с остальным API модуля, а не в тематическом
  («Итераторы и генераторы», «Исключения», «Списки (list)», «Функции», «ООП»);
- **встроенная функция/тип** остаётся в тематическом разделе
  («Типы данных», «Арифметика и операторы», «Циклы», «Ввод и вывод», …) и
  получает второй тег `builtin`. Раздел «Встроенные функции» — плоский каталог
  `functions.html`, поэтому при конфликте выигрывает тематика: иначе
  «Типы данных» осталась бы без `int`/`str`/`float`, а «ООП» — без
  `@property`/`super()`.

`docs_url` встроенных — всегда `library/functions.html#<имя>`, членов модулей —
`library/<модуль>.html#<qualname>` (совпадает с `docs_url_for()` из
`scripts/generate_draft_cards.py`).

Не дубли, хотя `id` оканчивается на `-1`: `any-1` — это `typing.Any` (а `any` —
встроенная функция), `in-not-in-для-множеств-o-1` и `срезы-с-шагом-2-1` — слаги
заголовков с `O(1)` и `[::2] / [::-1]`.

## Комплектная база (bundled)
...
`python -m stepik_grader.glossary.coverage` — не хардкод; черновиков-автодрафтов
нет, фильтр статусов скрывает лишь приватные не-`ready` имена; по файлу на цветовую группу `cg`), частично импортированная из внешнего
...
### Черновики из официальной документации
...
`str.split` тем самым закрывает свой coverage-пробел. `summary` пуст —
...
дефолтный вид показывает только `ready`, черновики доступны
...
**Состояние базы.** Автодрафтов не осталось: stdlib доведён до `ready`,
покрытие офлайн-инвентаря ~100%, `data/drafts.json` пуст.
...
### Полуавтоматический конвейер черновиков
...
примеры прогоном** («# → результат» сверяется с фактическим
...
docstring, каркас B1). Реальный BYOK-LLM (облако/ollama на `requests`)
реализует тот же протокол и подключается **opt-in** — не входит в
...
числу строк вывода), `skipped` (карточка не для этой ОС, см. ниже). Битые
примеры блокируют запись черновика.

**Как писать ожидание.** Сверка терпима к тому, как значение
записано руками, но не к самому значению:

| Запись ожидания | Что сверяется |
|---|---|
| `# → (1,2,3)` | как литерал: пробелы после запятых не важны, `'P'` ↔ `P` |
| `# → 3.14159...` | префикс до многоточия |
| `# → True (за O(1))` · `# → False — F_OK это проверка` | значение до пометки; пометка в скобках, через тире, двоеточие или запятую |
| `# → TypeError` | **намеренное падение**: пример исполняется по операторам, пойманное исключение печатается именем, и строки после демонстрации тоже сверяются |

Пример, который зовёт API за пределами POSIX (`os.fork`, `os.statvfs`,
`/dev/tty`), помечается у карточки тегом `platform:posix`: вне POSIX она
`skipped`, иначе вердикт говорил бы об ОС прогона, а не о карточке.

**Пропуск тега ловится машинно.** `platform_gaps()` разбирает примеры (AST) и
сверяет обращения к системным модулям с их фактическим составом через
`hasattr`, а `import pwd`/`termios` — с закрытым списком POSIX-only модулей.
Признак — код примера, не русский текст ожидания: пометка «только Unix» словами
ничего не гарантирует. Карточка без тега, но с таким обращением попадает в
отчёт `check` строкой `[нет тега]`, а тест `test_bundled_cards_calling_absent_api_are_tagged`
не даёт её смержить. Не считаются обращением: имена под `hasattr(os, 'fork')`
(OS-робастный пример печатает `True` на любой ОС), выдуманные имена
(`from os import nonexistent` — демонстрация `ImportError`, а не платформы) и
API новее интерпретатора (`os.readinto` есть с 3.14, и на 3.12 его нет на любой
ОС — платформа тут ни при чём). Последние два отсекаются сверкой с базой: имя
должно быть `id` карточки, а её поле `version` — не выше текущей версии Python.
Вне POSIX проверка видит все гэпы, на POSIX — только непереносимые импорты,
зато там примеры и исполняются целиком.

Каждая карточка исполняется в своём subprocess, поэтому `check` по всей базе
обходит карточки параллельно (по числу ядер; `--jobs N` задаёт вручную,
`--jobs 1` — последовательно). Отчёт от этого не «плывёт»: строки печатаются в
порядке отсортированных `id`, а не завершения прогонов.
