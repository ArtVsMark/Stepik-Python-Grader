...
_GROUPS: tuple[tuple[str, str, str, bool], ...] = (
    ...
    (
        "document",
        "Держится документом",
        "Нарушение заметит человек — если читал. Утверждение здесь и есть то, "
        "что он должен был прочесть.",
        True,
    ),
    (
        "pipeline",
        "Держится конвейером",
        "Нарушение заметит прогон, но слияние не удержит: ночной обход, "
        "неблокирующая проверка, задача-адресат.",
        True,
    ),
    (
        "gate",
