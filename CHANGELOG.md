...
### Added
- Глоссарий: EN-переводы `summary` для карточек старого импорта из Glossary-Python — при `?lang=en` карточка больше не откатывается молча на русский; прогресс держит EN-ratchet `scripts/audit_glossary_cards.py` + тест `tests/test_glossary_card_audit.py` (#684)
