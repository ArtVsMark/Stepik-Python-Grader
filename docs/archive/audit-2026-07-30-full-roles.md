...
- **Документ отработан и заархивирован — состояние находок здесь не ведётся.** Строка на этом месте годами
  обещала обратное («все находки открыты, номер PR проставляется по мере закрытия»), и обещание пережило и
  закрытие эпика **#771**, и сам переезд в архив: читатель открывал заведомо отработанный документ и видел
  «открыта» напротив каждой строки, а брал в работу давно сделанное (находка `AUD-3-01`, issue #962).

  Состояние находки читают **из задачи**, а не отсюда: реестр документа аудита состоянием не является и
  руками не ведётся — он собирается один раз, при архивации ([CLAUDE.md § Открытая работа](../../CLAUDE.md)).
  Этот аудит закрыт целиком: эпик **#771** с одиннадцатью подэпиками, см. строку в
  [индексе архива](README.md).
...
**Закрыто:** `DESW-04` — PR #859 (плейсхолдеры обоих редакторов, stdin, пути и заголовка обратной связи переведены; подсказка contenteditable получила собственный ключ-компаньон). `DESW-02` — PR #872 (глоссарий, правила, «Подучить» и «Прогресс» различают сбой и пустоту: сообщение с кнопкой «Повторить» вместо «Пока пусто — и это отлично 🎉»). `DESW-01`, `DESW-03`, `DESW-05`, `DESW-06`, `DESW-07` — PR #909.

> `DESW-01` закрыт не подкруткой токена. Минимальное значение `--color-text-faint`,
> проходящее 4.5:1 на всех поверхностях, — `#676663` (light) и `#938f8c` (dark),
> то есть практически `--color-text-muted` (`#67665f` / `#918f8b`): третьего
> уровня «бледнее muted» в доступной палитре не существует, и предложенные
> аудитом `~#8f8d84` / `~#7a7875` дают 2.6–2.8:1, а не AA. Поэтому восемь
> текстовых применений переведены на `muted`, а токен переименован в
> `--color-faint` и оставлен двум нетекстовым (скроллбар, левая рамка карточки) —
> имя `text-*` у нетекстового токена и было ловушкой. Само упущение закрыто
> проверкой `check_pairs_cover_text_tokens`: токен в `color:` без пары в `_PAIRS`
> теперь роняет гейт (см. `DESW-N1` ниже — в этой части выполнено).

> `DESW-06` реализован без снятия `pointer-events: none` со стека (решение
> issue #633 в силе — уведомление не перекрывает кнопки под собой): события
> принимает сам тост, промежутки между тостами остаются прозрачными.
...
**Закрыто:** `FER-01` — PR #871 (опрос проверяет статус ответа и наличие `progress`, тело цикла защищено с гарантированным завершением; вместо вечного спиннера — сообщение «Прогон не найден» и разблокированные кнопки). `FER-02`, `FER-04` — PR #906 (токен отмены навигации `_routeSeq`; слушатель онбординга на `document` + закрытие кликом по подложке). `FER-03` — PR #908: `setSection` пишет раздел в `location.hash` через `replaceState` в формате ссылок сайдбара (`#check`), роутер получил ветку на раздел, карточки (`#/glossary/<id>`, `#/rules/<code>`) адресуют точнее и разделом не переписываются.

> `FER-03` входил и в PR #906, но был оттуда откачен: первая реализация вводила
> второй формат адреса (`#/check`) и синхронизировала хэш на старте — до того,
> как роутер разберёт прямую ссылку. Это уронило 18 e2e. Поймано прогоном
> `tests/e2e/` (он ставится opt-in-экстрой `[e2e]` и по умолчанию пропускается —
> отсюда и уверенность, что «фронтенд отсюда не проверить»).
...
**Закрыто:** `FED-03`, `FED-04` — PR #872 (`loadAuthStatus` смотрит на HTTP-статус, а не только на тело; сбой конфига виден строкой с кнопкой повтора). `FED-01` — PR #910: у опроса появился токен отмены (приём `state.activeRunId`/`_routeSeq`), узлы панели ищутся на каждом шаге, «Отмена» в мастере останавливает цикл явно.

> Отмену по *любой* перерисовке панели делать нельзя, хотя предложение аудита
> читается именно так: `renderAuthPanel()` вызывается и при возврате в раздел,
> и тогда OAuth, идущий в фоне, остался бы без опроса — а до этой правки его
> `done` всё же доводил дело до `loadAuthStatus()`. Поэтому цикл вытесняется
> только новым запуском или явной «Отменой».
...
**Закрыто:** `FES-01`, `FES-02`, `FES-03` — PR #911 (кнопка отмены гаснет только на время запроса и возвращается с сообщением при отказе; ссылки на форму гасятся до прихода нового черновика, а не только при его провале; у обновления черновика появился токен актуальности `_draftSeq`).

> `FES-03` сторожит статическая проверка (`tests/test_web_feedback_draft_guard.py`),
> а не e2e: доставить поздний ответ на ранний запрос из Playwright
> детерминированно не выходит — придержанный `route.fulfill()` до страницы уже
> не доезжает, и такой сценарий зеленеет и без фикса (проверено откатом).
> Остальные две находки проверены живыми сценариями.
...
> **Колонки «Заявлено» и «Чем опровергнуто» обрезаны при сборке документа.**
> Обрыв виден по строкам, кончающимся посреди слова («…к неStepik-», «…cli/__init__.py:94 и т»).
> Полного текста не существует: обрезка присутствует уже в первой ревизии файла
> (`de5a02d`), то есть текст потерян не правкой, а генератором — восстанавливать
> из истории нечего. Ревизия аудита 2026-08-10 (находка REV-5-01) требовала
> «починить реестр»; починить его дословно нельзя, поэтому здесь стоит эта
> пометка — чтобы следующая ревизия не искала несуществующее.
>
> **Как перепроверять отклонение:** по ID находки и указанным в правой колонке
> `file:line` — они уцелели у всех строк и ведут к живому коду. Обрезан хвост
> обоснования, а не его начало и не адрес.
>
> Причина устранена на будущее: правило «обрезать — только с явным маркером»
> добавлено в [docs/agent/multiagent.md](../agent/multiagent.md).
