...
3. **RU/EN (soft, ratchet).** Число карточек без ``summary_en`` не должно расти
   выше ``MAX_CARDS_WITHOUT_EN`` — перевод старого импорта вынесен отдельной
   волной (#684), но новые карточки обязаны быть двуязычными, поэтому счётчик
   только убывает.
...
# Ratchet EN-полноты (issue #684). Ровно столько карточек старого импорта из
# Glossary-Python сейчас без summary_en; перевод — отдельной волной. Значение
# только УМЕНЬШАЕТСЯ по мере перевода; расти ему нельзя (новые карточки —
# двуязычны by design). Свести к 0 по завершении EN-волны.
MAX_CARDS_WITHOUT_EN = 525
