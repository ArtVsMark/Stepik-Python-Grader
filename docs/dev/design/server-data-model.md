...
> Дизайн-документ. **Не реализация**: описывает целевую
...
- [Accounts / workspaces / courses / tasks](#accounts--workspaces--courses--tasks)
- [Права доступа (RBAC)](#права-доступа-rbac)
- [PostgreSQL-схема поверх SQLite](#postgresql-схема-поверх-sqlite)
...
разделяемой БД (PostgreSQL). Требование — согласовать SQLite и PG **без
дублирования доменной модели** и спроектировать сущности
...
критерий «нет дублирования доменной модели».
...
## Accounts / workspaces / courses / tasks
...
2. **`test_set` версионируется**, а `run` ссылается на конкретную версию — иначе
   ...
   (та же проблема, что локально решает `solution_hash`/перескачивание).
...
## PostgreSQL-схема поверх SQLite
...
Отсутствие дублирования: доменные поля прогона/кейса/линта — **те
