...
Всё локально, ничего не уходит в сеть. Два файла в рабочей папке:

| Файл | Что внутри |
|---|---|
| `.grader_history.db` | журнал прогонов: задача, вердикты, `failure_kind`, **sha256** решения (не сам код), тайминги. Плюс спутники `-wal`/`-shm` |
| `.grader_stats.jsonl` | статистика запусков: режим, вердикты, ОС, длительность |

Оба создаются с правами «только владелец» (0600) — на Windows это no-op, там
...
Оба файла стоит держать в `.gitignore` репозитория с решениями — грейдер их
туда не добавляет.
