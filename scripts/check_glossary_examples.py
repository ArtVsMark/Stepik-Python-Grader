...
#: Сколько карточек ещё не компилируется. Число только уменьшается.
#:
#: 90 → 67 → 49 → 36 → 30 → 5 (issue #1454): партиями, руками. Пятая партия —
#: двадцать четыре карточки разом: `oop.json` целиком, все сортировки и
#: структуры данных `op.json`, исключения, итераторы, `dataclass`.
#:
#: ОСТАВШИЕСЯ ПЯТЬ — не «недоделанный остаток», а другая задача. `permissionerror`,
#: `open`, `pathlib.path`, `.iterdir`, `итерация-по-файлу` жёстко завязаны на
#: POSIX-пути (`/etc/shadow`, `/etc/hostname`, `/tmp/demo.txt`), и расстановка
#: отступов их не чинит, а ухудшает: пример становится исполнимым и падает на
#: Windows там, где раньше молча пропускался. Такой карточке нужен либо тег
#: `platform:posix`, либо кроссплатформенный пример (`tempfile` вместо `/tmp`) —
#: это смена самого примера, а не его отступов, и гарантия у неё другая.
#:
#: Автоматом отступы не расставляются, и попытка измерена: эвристика «после
#: двоеточия уровень +1» из блока никогда не выходит, поэтому она сворачивает
#: весь пример в лестницу вложенности — а `ast.parse` такой результат ПРИНИМАЕТ,
#: и прогон тоже (вложенные определения не исполняются): 80 карточек из 90
#: прошли приёмку, верной не была ни одна. Приёмка вышла слабее чинимого
#: дефекта, а цена ошибки несимметрична — сломанный пример виден,
#: «починенный» нет.
BUDGET = 5
