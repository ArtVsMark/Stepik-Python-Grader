...
- **`CHANGELOG.md`** (русский) — запись под `## [Unreleased]` в **каждом**
  ...
  - **Пишем в БУФЕР, а не в секцию.** Новая запись идёт в `## Буфер (разобрать
    при релизе)` строкой вида `- Fixed: что изменилось (#PR)` — секция помечена
    в самой строке. Так параллельные PR не дерутся за одно место: git сливает
    `CHANGELOG.md` обеими сторонами (`merge=union` в `.gitattributes`), а раз в
    буфере нет секций и порядок строк не значим, запись не может уехать не в
    тот раздел. Разбор буфера по секциям — при релизе, вместе с ротацией.
    Правило появилось после того, как четыре параллельных PR дали конфликт на
    каждый мерж (все — «обе стороны дописали свою строку») и развели в
    `[Unreleased]` две секции `### Added`.
