...
AI-подсказки в web (`POST /api/v1/hint`, issue #543) — opt-in и **гейтятся
согласием**: объяснение упавшего кейса отправляет код решения и его ввод-вывод
стороннему AI-провайдеру (`ai_base_url`), поэтому первый запрос требует явного
однократного согласия (иначе **403** `consent_required`, в сеть ничего не уходит).
Согласие запоминается в `.grader_settings.json` (`ai_hint_consent`). Рекомендация
— локальный ollama (данные не покидают машину); для несовершеннолетних — согласие
представителя. Провайдер не настроен → тихий пропуск.
...
- **Prompt-injection (issue #692, характеризовано).** Подконтрольный решению
