...
Потребители этих форм — три: клиентские ES-модули (`web/static/js/*`),
шаблонизация страницы и тесты, сверяющие вид с этим документом. Расходятся они
тихо, поэтому правила те же, что у [ядра результата](result-contract.md), и
поверх — то, что специфично для веба.

1. **Стабильны имена полей view-моделей и значения перечислений.**
   `kind`, `mode`, `status`, `verdict`, `id` команды — имена, на которые
   ветвится клиент; переименование или смена смысла означает правку всех трёх
   потребителей одним PR, а не «клиент подстроится».
2. **Расширение аддитивно, незнакомое поле клиент игнорирует.** Новое поле
   view-модели добавляется необязательным, а незнакомое значение перечисления
   рисуется нейтрально — падать на нём клиент не вправе: сервер и страница
   обновляются не одновременно (открытая вкладка переживает перезапуск).
3. **Как добавляют новое.** Поле сначала появляется в adapters-слое
   (`web/*_adapter.py`, `web/viewmodels.py`), затем в псевдо-схеме этого
   раздела, затем у клиента; команда — ещё и записью в реестре команд выше.
   Новая строка интерфейса заводится в `ui.json` сразу в обеих локалях —
   иначе гейт `check_ui_locale_guardrails.py` покажет расхождение.
4. **Ошибка — это `kind: "error"` с `message`, а не исключение формы.**
   Набор состояний экрана пополняется, структура ответа — нет.
