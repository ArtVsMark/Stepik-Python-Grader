...
42 с последовательно против 2 мин 30 с в параллель. В `addopts` флаг не
прописан намеренно, и решение о нём принимает тот, кто запускает: два теста
`TestGradeBenchmarkReference` (`tests/test_web.py`) сравнивают **относительную
скорость** решений, и под шестью воркерами вердикт `SLOWER`/`FASTER` плавает —
красное там означает загруженный процессор, а не регрессию. Гейт
(`preflight.py`) и CI остаются последовательными, поэтому «зелено» у них
означает то же, что и раньше.
