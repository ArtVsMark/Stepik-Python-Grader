...
#: Сколько карточек ещё не компилируется. НОЛЬ — и это уже не «разрешённый
#: долг», а запрет: любая новая карточка с несобирающимся примером краснеет
#: сразу, а не когда-нибудь.
#:
#: 90 → 67 → 49 → 36 → 30 → 5 → 0 (issue #1454). Первые пять партий — ручная
#: расстановка отступов, где гарантией было «`strip()` каждой строки совпадает
#: с оригиналом». Последняя пятёрка чинилась иначе и потому шла отдельно:
#: `permissionerror`, `open`, `pathlib.path`, `.iterdir`, `итерация-по-файлу`
#: были завязаны на POSIX-пути (`/etc/shadow`, `/etc/hostname`, `/tmp/demo.txt`),
#: и расстановка отступов их не чинила, а ухудшала — пример становился
#: исполнимым и падал на Windows там, где раньше молча пропускался. Им нужен
#: был другой пример, а не другой отступ.
#:
