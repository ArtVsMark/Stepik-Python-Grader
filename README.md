...
> Автоматическая проверка и сравнение Python-решений с курсов Stepik.
> Тестирование через `subprocess` и `timeit`, 4 рабочих режима.
...
**Stepik Python Grader** — инструмент для локальной проверки Python-решений задач с платформы [Stepik](https://stepik.org).
...
Проверяет одно решение против его тест-кейсов. Выводит количество пройденных тестов, время и память.
...
Запускает N повторений для каждого решения через отдельный процесс. Выводит min/median/mean/max/std-dev и сравнивает скорость решений относительно быстрейшего.
...
Файл `diagnostik_stepik.py` позволяет:
