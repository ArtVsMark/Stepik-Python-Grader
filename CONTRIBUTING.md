...
> **UX вывода `--version`.** Сейчас `--version` показывает сырые метаданные
> пакета (включая `.postN+g<hash>` вне тега). Улучшение читабельности —
> явный маркер dev vs release — это задача **#163** (см.
> [`docs/claude-handoff.md`](docs/claude-handoff.md)), в этом файле только
> политика.
