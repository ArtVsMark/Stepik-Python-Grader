...
- **Run result** — обёртка над набором решений с полями `kind`
  ...
  относительных имён), `rows` (по решению), `message` (только при
  `kind="error"`). Это уже адаптерный слой; Web отдаёт его как
  `ResultViewModel` — см. [web-mvp.md](web-mvp.md#resultviewmodel).
