...
### `merge-when-green` — согласие смержить без вас

Метка **`merge-when-green`** на pull request означает «как позеленеет, мержи без
меня». Её видит `merge-when-green.yml` и включает PR авто-мерж (squash);
дальше работают уже существующие механизмы — проверки, обновление головы
очереди, мерж по зелёному.

Это единственная метка, которая **меняет поведение конвейера**, а не только
навигацию, поэтому у неё свои правила:

- ставит её **человек** — согласие на мерж должно быть выражено явно. Защита
  ветки проверяет качество, но не намерение: включи авто-мерж всем подряд, и в
  `main` уедет PR, который автор ещё хотел посмотреть;
- **решение обратимо**: снятая метка выключает авто-мерж, пока PR не смержен;
- черновики и PR из форков пропускаются — у первых согласия ещё нет, вторые
  ведут внешние авторы.

Без метки ничего не меняется: PR, открытый вручную, ждёт человека, как и
раньше. Метку заводит владелец репозитория — скрипт её только читает.
