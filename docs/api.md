...
#321/#322, панель «Функции в коде» песочницы и режимов 1/2). Тело — либо
`{"code": "..."}` (режим 1/песочница, debounce при редактировании), либо
`{"path": "..."}` (режим 2, разово по выбранному решению — путь конфайнится в
workspace и читается).

Сканирует код (`scan_code_concepts` — повседневные builtin'ы
`CODE_TERM_BUILTINS`, импортированные функции, методы встроенных типов
`x.split()`, `match/case`) и сопоставляет с карточками базы по
