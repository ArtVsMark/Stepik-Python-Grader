...
**Непригодный путь.** До резолва путь проверяется на то, что его вообще можно
отдать файловой системе: строка с NUL-байтом — **400** с
`message_id: path_not_usable` и полем `reason`. Проверка идёт **до** ветки
конфайнмента и не отключается `--no-root-confinement`: прежде такой путь ронял
обработчик `ValueError`-ом, и соединение закрывалось без единого байта ответа —
клиент видел обрыв вместо причины.

**Неизвестный сбой.** Необработанное исключение хендлера превращается в
**500** с `message_id: server_internal_error` (поля `route`, `error`), а не в
разрыв соединения: одностраничный интерфейс иначе остаётся в состоянии «идёт
запрос», и единственным выходом становится перезагрузка. Полный трейсбек уходит
в лог сервера. Если ответ уже начат, второй не отправляется — дописать статус
поверх отправленных заголовков нельзя.
...
Синхронный грейд занимает место в **общем** учёте активных прогонов: при
`CONFIG.max_active_runs` активных (считая async-job'ы) — **429**
`too_many_runs`, тем же кодом и сообщением, что у `POST /api/v1/runs`. Отмены и
TTL у этого пути нет и быть не может: запрос держится открытым всю длительность
прогона, отменять нечего, результат никуда не складывается — для отмены есть
асинхронный путь.
...
Отметка перехода в карточку глоссария **из ошибки** прогона — deep-link из панели разбора, а не открытие раздела «Глоссарий» руками. Тело:
...
Потребитель этого API — не только собственный веб-интерфейс: под `--serve`
на него смотрит и соседний инструмент, и он обязан знать, на что вправе
опереться, а что завтра станет другим.

1. **Стабильны путь, метод и смысл полей ответа у маршрутов `/api/v1/*`.**
   Переименование поля, смена типа или смысла — ломающее изменение: путь
   получает следующую версию (`/api/v2/...`), а старый живёт, пока есть
   потребители. Маршруты без `v1` в пути (`/api/glossary`, `/api/task/*`,
   `/api/download`) обслуживают собственную страницу, версии не несут и
   меняются вместе с ней — опираться на них снаружи не следует.
2. **Расширение аддитивно, незнакомое поле клиент игнорирует.** Новые ключи
   ответа и новые необязательные параметры запроса добавляются без смены
   версии; клиент, падающий на незнакомом ключе, написан неверно. То же и с
   `message_id`: набор пополняется, поэтому неизвестный идентификатор
   показывается как есть, а не считается ошибкой протокола.
3. **Как добавляют новое.** Маршрут объявляется в таблице
   `_API_GET_EXACT`/`_API_POST_EXACT`
   ([`web/api_routes.py`](../../src/stepik_grader/web/api_routes.py)), в
   ядро ходит через адаптер (ARCH-07, [ADR-0010](adr/0010-web-core-boundary.md)),
   описывается разделом в этом файле и попадает в оглавление — контракт-тест
   сверяет разделы с таблицей маршрутов и падает на расхождении. Новый
   `message_id` заводится сразу в обеих локалях (`ru`/`en`).
4. **Ответ об ошибке не меняет формы.** `{"kind": "error", "message_id": ...,
   "message": ...}` и HTTP-код — часть контракта наравне с успешным ответом;
   пополняется набор идентификаторов, а не структура.
