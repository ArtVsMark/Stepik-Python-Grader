...
> [web-contracts.md § Безопасность](../web-contracts.md#безопасность-и-локальное-исполнение) и
...
- `RunSpec` — что запустить: путь решения (или сгенерированного
  ...
  а также `script_name` и `aux_files` (issue #992). Последние два существуют
  потому, что изоляция видит **только** материализованное в её рабочем
  каталоге: под каким именем лечь скрипту и какие файлы обязаны лежать рядом,
  знает вызывающая сторона, а не backend. Пока backend'ы решали это сами
  (всегда `solution.py`), сгенерированная обёртка ложилась под именем модуля
  решения и импортировала саму себя — верное решение получало `FAIL` на всех
  function-кейсах.

  **Контракт паритета.** Вердикт кейса обязан совпадать с изоляцией и без неё:
  `--sandbox` меняет способ исполнения, а не результат проверки. Закреплено
  `tests/test_sandbox_parity.py` — расхождение красит тест. До появления этого
  контракта изоляция была вторым исполнителем со своим поведением, и шесть
  заявленных `high` аудита 2026-08-10 ломали именно верные решения.
...
- **Не выбирать конкретный sandbox-рантайм** (runc/crun/gVisor/Firecracker) —
  это решение деплоя. *Класс* backend (OS-контейнер + cgroups v2 + netns +
  seccomp) сужен [ADR-0008](../adr/0008-server-sandbox-backend.md), но конкретный
  рантайм и его установка — этап реализации.
