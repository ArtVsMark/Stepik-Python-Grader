...
Пункт `9` — обратная связь (эпик #751): спрашивает тип обращения (баг / идея /
задача Stepik проверяется неправильно) и краткое описание, добавляет
автособранное окружение (версия, ОС, Python, состояние `--sandbox`), печатает
**предпросмотр того, что уйдёт**, и только после подтверждения открывает браузер
с уже заполненной формой issue на GitHub. Грейдер ничего не отправляет сам:
Submit жмёт пользователь. Код решения и токены в обращение не попадают, домашний
путь сворачивается в `~`. Тот же канал в веб-интерфейсе — кнопка 💬 в topbar
(см. [web-current.md § Обратная связь](../dev/web-current.md#обратная-связь-кнопка--в-topbar-issue-754)).
