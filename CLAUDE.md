...
- **Одно окно на GitHub API за раз.** Квота общая на аккаунт, и цикл ожидания
  ...
  конвейер.
- **Окно старше нескольких дней работает по устаревшим правилам.** И `CLAUDE.md`,
  и `.claude/settings.json` читаются **при старте сессии** и не перечитываются:
  правка, приехавшая в `main` позже, до запущенного окна не дойдёт. Прецедент:
  недельное окно продолжало ходить через MCP спустя сутки после того, как
  правило сменилось, и в одиночку выжгло квоту вдвое сверх лимита — тогда как
  все окна, запущенные позже, ходили по REST. Поэтому долгоживущее окно
  **закрывается, а не дорабатывает**, а после изменения правил транспорта
  активные окна перезапускаются — иначе механизм действует только на новые.
...
1. **Обновить ветку из `main`** (`python scripts/gh_rest.py update-branch <PR>`
   или кнопка Update branch). Устаревшая ветка проверена на состоянии,
   которого уже нет.
2. **Включить auto-merge на PR** — `python scripts/gh_rest.py auto-merge <PR>`.
   Штатная функция, доступная и личному репозиторию: PR смержится сам, как
   только пройдут обязательные проверки. Именно она снимает нужду сидеть и
   опрашивать статусы — а опрос и есть то, что дважды за день выжигало GraphQL
   до нуля.
