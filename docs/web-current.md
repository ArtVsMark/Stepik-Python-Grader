...
> код с нуля. По ▶ код сначала сохраняется на диск
> (`POST /api/save-solution`) — в выбранный файл, если он был выбран, иначе в
> новый файл по маске `task<опц.цифры>_<M>.py`
> (`web/viewmodels.py::_next_solution_filename`) — и только потом запускается
> обычная проверка сохранённого пути, без какого-либо сравнения/ранжирования.
