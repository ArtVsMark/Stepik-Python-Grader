...
(OS-робастный пример печатает `True` на любой ОС) и выдуманные имена
(`from os import nonexistent` — демонстрация `ImportError`, а не платформы):
второе отсекается сверкой с `id` карточек базы. Вне POSIX проверка видит все
гэпы, на POSIX — только непереносимые импорты, зато там примеры и исполняются
целиком.
