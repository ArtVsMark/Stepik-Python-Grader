...
### Два пути запуска — терминал и окно

После установки доступны обе команды, и обе работают из любого каталога:

| Команда | Что открывает | Кому |
|---|---|---|
| `stepik-grader` | интерактивное меню в терминале (пункты 0–9) | привычно тем, кто и так живёт в консоли |
| `stepik-grader-gui` | окно-лаунчер: режим сервера, порт, рабочая папка | тем, кому терминал не нужен |

На Windows `stepik-grader-gui` — это `pythonw`-исполняемый файл **без консольного
окна**: двойной клик открывает лаунчер, а не чёрный экран. Из исходников то же
окно поднимается как `python -m stepik_grader.launcher`.

### Ярлык в системе

Чтобы не набирать команду каждый раз:

```bash
stepik-grader --create-shortcut     # напечатает путь созданного ярлыка
```

То же делает кнопка «Создать ярлык на рабочем столе» в самом лаунчере — она
включается после первого успешного старта сервера, когда уже видно, что именно
кладётся на рабочий стол.

| ОС | Что создаётся | Куда |
|---|---|---|
| Windows | `.lnk` (через PowerShell) | рабочий стол |
| macOS | исполняемый `.command` | рабочий стол |
| Linux | `.desktop` | `~/.local/share/applications` — ярлык появляется в меню приложений |

Ярлык ведёт на `stepik-grader-gui`, а не на путь внутри venv, поэтому переживает
переустановку пакета. Повторный вызов перезаписывает существующий ярлык, а не
плодит копии. **Без явного действия ярлык не создаётся никогда** — ни при
установке, ни при первом запуске.

### Запуск из клона

Если репозиторий склонирован, в корне лежат `launcher.sh` (Linux/macOS) и
`launcher.cmd` (Windows): они находят `.venv` рядом с собой и поднимают то же
окно, не требуя `activate`. Без `.venv` печатают, что именно сделать, а не
трассировку.
...
**Если диагностика не прошла**, она называет причину и следующий шаг прямо в
консоли, а рядом кладёт `stepik_diagnostics/environment_checks.json` — отчёт по
проверкам окружения (`secrets.json`, данные OAuth-приложения, сохранённый
токен, свободен ли порт колбэка, отвечает ли Stepik). Секреты в нём скрыты, и
файл предназначен для того, чтобы приложить его к issue: понимать его
