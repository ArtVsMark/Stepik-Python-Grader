...
### Internal
- CI: `ruff` (dev-extra без верхней границы) доехал до 0.16, который стал форматировать Python в Markdown и валил `ruff format --check` на любом PR из-за код-блоков в `CLAUDE.md`/`docs/*.md`; Markdown исключён из области ruff через `[tool.ruff] extend-exclude` (#684)
