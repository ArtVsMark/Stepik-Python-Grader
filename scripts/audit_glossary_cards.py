...
1. **Структура (hard).** У каждой ``ready``-карточки есть минимально обязательный
   ...
   все карточки базы; фиксируем его инвариантом, чтобы база не разъехалась.
...
ЧЕГО ЭТОТ СКРИПТ НЕ ПРОВЕРЯЕТ — и где это проверяется. Примеры карточек он не
компилирует и не исполняет: ни ``compile``, ни ``ast.parse``, ни запуска здесь
нет. Аудит 2026-08-10 записал это находкой ``ADD-3-07`` именно потому, что
искал проверку примеров тут — по имени файла разумно ожидать, что «ревизия
карточек» смотрит и на их код.

Проверка есть, но в двух других местах, и разделение намеренное: одна и та же
вещь, проверяемая трижды, даёт три источника правды и расходится молча.

* ``scripts/check_glossary_examples.py`` — пример обязан КОМПИЛИРОВАТЬСЯ.
  Бюджет нулевой (issue #1454), то есть несобирающийся пример роняет CI сразу.
* ``tests/test_glossary_draft_pipeline.py::test_run_check_bundled_ratchet`` —
  пример ИСПОЛНЯЕТСЯ, и фактический вывод сверяется с заявленным в ``# → …``.
  Карточки, чьё расхождение ещё не разобрано, перечислены в
  ``tests/glossary_audit_known_issues.txt``; список только сокращается.

Здесь — структура, достижимость детектором и RU/EN. Это разные вопросы к одним
данным, и держать их порознь дешевле, чем сводить в один скрипт.
