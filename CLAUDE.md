...
> #71 ✅ (2026-07-06): `--watch --mode 2` стал инкрементальным, переиспользуя
> кэш #56 (как и предлагал issue — вместо отдельного in-memory решения). Под
> `--watch` кэш включается автоматически (`_resolve_use_cache(args,
> incremental=args.watch)`), поэтому на событие перепрогоняется только
> изменённый файл, остальные строки берутся из `.grader_cache/`; между
> итерациями watch кэш живёт через диск (каждый `_run_mode_2` грузит его при
> старте и сохраняет в конце — существующая плумбинг #56, править не пришлось).
> Дефолт `--cache` изменён с `CONFIG.use_cache` на `None`, чтобы отличать явные
> `--cache`/`--no-cache` (всегда выигрывают) от «не задано»; приоритет —
> `_resolve_use_cache`: явный флаг → watch-инкрементальный дефолт →
> `[tool.stepik-grader] use_cache`. `--mode 1` (один файл) кэш под `--watch`
> автоматически НЕ включает — инкрементальность там бессмысленна.

> IDE launch fix (2026-07-06): задачи `--init-vscode` и рецепт PyCharm в README
> запускали грейдер через голый `stepik-grader` — он в PATH только при
> активированном venv, из-за чего у новичка задачи «не запускались». Теперь
> VS Code-задачи вызывают `${command:python.interpreterPath} -m
> stepik_grader.grader …` (`type: process` — путь интерпретатора может
> содержать пробелы, process не квотит аргументы через шелл), а PyCharm
> External Tool — `$PyInterpreterDirectory$/python -m stepik_grader.grader …`.
...
## 📊 МЕТРИКИ ПРОЕКТА (main, post-v1.4.0 — не выпущено)

| Метрика | Значение |
|---------|---------|
| Версия | 1.4.0 (последний релиз; в main — unreleased #56/#57/#71 + IDE-фикс) |
...
[ ] ВЕРСИЯ согласована по всему проекту (перед КАЖДЫМ мержем):
    pyproject.toml [project].version, README (бейдж version-X.Y.Z, «Status:
    vX.Y.Z», примеры вывода `--version`), CLAUDE.md таблица метрик — не
    противоречат друг другу; устаревших номеров версий в доках нет.
    Быстрая проверка: grep -rnE "[0-9]+\.[0-9]+\.[0-9]+" README.md CLAUDE.md
    pyproject.toml | grep -v CHANGELOG   (сверить с текущим релизом)
```

> Правило (запрошено 2026-07-06): перед КАЖДЫМ мержем — проверять
> соответствие версии по всему проекту (см. чек-бокс выше). Схему версий не
> меняем (тег = vX.Y.0, PATCH = коммиты после тега, `scripts/version.py`);
> следим лишь за тем, чтобы номера версий в README/CLAUDE.md/pyproject не
> расходились между собой и с последним релизом. Устаревшие номера
> (например, README `# 1.2.0` в примере `--version`) — чинить в том же PR.
