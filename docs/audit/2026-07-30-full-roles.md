...
**Закрыто:** `FER-01` — PR #871 (опрос проверяет статус ответа и наличие `progress`, тело цикла защищено с гарантированным завершением; вместо вечного спиннера — сообщение «Прогон не найден» и разблокированные кнопки). `FER-02`, `FER-04` — PR #906 (токен отмены навигации `_routeSeq`; слушатель онбординга на `document` + закрытие кликом по подложке). `FER-03` — PR #907: `setSection` пишет раздел в `location.hash` через `replaceState` в формате ссылок сайдбара (`#check`), роутер получил ветку на раздел, карточки (`#/glossary/<id>`, `#/rules/<code>`) адресуют точнее и разделом не переписываются.
