...
| **Песочница** | Песочница | Не проверка, а «запусти этот код с этим вводом»: редактор + поле stdin + вывод (stdout/stderr, код выхода, время). Статусы OK/RE/TLE/CANCELLED (issue #317) | `web/playground.py::run_playground()` поверх `core/runner.LocalRunner`; async через `POST /api/v1/runs` `mode="playground"` |
